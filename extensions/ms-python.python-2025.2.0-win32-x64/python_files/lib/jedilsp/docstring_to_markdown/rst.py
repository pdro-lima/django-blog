...
SPHINX_PARAM = ("param", "parameter", "arg", "argument", "key", "keyword")
...
SPHINX_RULES: List[Directive] = [
    ...
    Directive(
        pattern=r":cpp:({}):`\.?(?P<name>[^`]+?)`".format(
            "|".join(SPHINX_CROSS_REF_CPP)
        ),
        replacement=r"`\g<name>`",
    ),
    Directive(
    ...
    Directive(
        pattern=r"(:py)?:({}):`\.?(?P<name>[^`]+?)`".format(
            "|".join(SPHINX_CROSS_REF_PYTHON)
        ),
        replacement=r"`\g<name>`",
    ),
    Directive(
        pattern=r"(:rst)?:({}):`\.?(?P<name>[^`]+?)`".format(
            "|".join(SPHINX_CROSS_REF_RST)
        ),
        replacement=r"`\g<name>`",
    ),
    Directive(
    ...
    Directive(
        pattern=r"^\s*:({}) (?P<type>\S+) (?P<param>\S+):".format(
            "|".join(SPHINX_PARAM)
        ),
    ...
    Directive(
        pattern=r"^\s*:(return|returns):", replacement=r"- returns:", flags=re.MULTILINE
    ),
    Directive(
...
ADMONITIONS = [
    Admonition(name="caution", label="Caution", icon="⚠️ "),
    Admonition(name="attention", label="Attention", icon="⚠️ "),
    Admonition(name="danger", label="Danger", icon="⚠️ "),
    Admonition(name="hint", label="Hint", icon="🛈"),
    Admonition(name="important", label="Important", icon="⚠️ "),
    Admonition(name="note", label="Note", icon="🛈"),
    Admonition(name="tip", label="Tip", icon="🛈"),
    Admonition(name="warning", label="Warning", icon="⚠️ "),
]
...
ADMONITION_DIRECTIVES: List[Directive] = [
    ...
    Directive(
        pattern=rf"\.\. {admonition.name}::", replacement=admonition.inline_markdown
    )
...
RST_DIRECTIVES: List[Directive] = [
    ...
    *ADMONITION_DIRECTIVES,
    ...
    Directive(
        ...
        + m.group("label")
        + "]("
        + re.sub(r"\s+", "", m.group("url"))
        + ")",
    ),
    Directive(pattern=r":mod:`(?P<label>[^`]+)`", replacement=r"`\g<label>`"),
    Directive(
        pattern=r"\.\. currentmodule:: (?P<module>.+)(?P<end>$|\n)", replacement=""
    ),
    Directive(pattern=r":math:`(?P<latex>[^`]+?)`", replacement=r"$\g<latex>$"),
    Directive(
...
ESCAPING_RULES: List[Directive] = [
    Directive(pattern=r"(?<!`)__(?P<text>\S+)__(?!`)", replacement=r"\_\_\g<text>\_\_")
]


def _find_directive_pattern(name: str):
    return [directive for directive in RST_DIRECTIVES if directive.name == name][
        0
    ].pattern
...
class TableParser(IParser):
    ...
    def _split(self, line: str) -> List[str]:
        ...
        for i, start in enumerate(self._column_starts):
                ...
                if i < len(self._column_starts) - 1
                else self._columns_end
            )
    ...
    def _wrap(self, row: List[str], align=str.ljust) -> str:
        padded_row = [align(e, self._max_sizes[i]) for i, e in enumerate(row)]
        return self._indent + "| " + (" | ".join(padded_row)) + " |\n"

    def finish_consumption(self, final: bool) -> str:
        result = self._wrap(self._columns, align=str.center)
        result += self._wrap(["-" * size for size in self._max_sizes])
...
class GridTableParser(TableParser):
    ...
    def can_consume(self, line: str) -> bool:
        return bool(self._state != TableParser.State.FINISHED) and (
...
class NoteBlockParser(IndentedBlockParser):
    enclosure = "\n---"
    directives = {f".. {admonition.name}::": admonition for admonition in ADMONITIONS}
...
RST_SECTIONS = {
    section: "\n" + section + "\n" + "-" * len(section) for section in _RST_SECTIONS
}

DIRECTIVES = [*RST_DIRECTIVES, *ESCAPING_RULES]
...
def rst_to_markdown(text: str, extract_signature: bool = True) -> str:
    ...
    def flush_buffer():
        ...
        for directive in DIRECTIVES:
            lines = re.sub(
                directive.pattern, directive.replacement, lines, flags=directive.flags
            )
    ...
    for line in text.split("\n"):
        ...
        if not active_parser:
            ...
            match = re.match(
                r"^(?P<indent>\s*)(?P<argument>[^:\s]+) : (?P<type>.+)$", line
            )
            if match:
                line = (
                    match.group("indent")
                ...
                if most_recent_section in SECTION_DIRECTIVES:
                    for section_directive in SECTION_DIRECTIVES[most_recent_section]:
                        if re.match(section_directive.pattern, trimmed_line):
                            line = re.sub(
                                section_directive.pattern,
                                section_directive.replacement,
                                trimmed_line,
                            )
                            break
