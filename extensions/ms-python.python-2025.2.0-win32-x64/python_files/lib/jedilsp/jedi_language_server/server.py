...
class JediLanguageServerProtocol(LanguageServerProtocol):
    ...
    def lsp_initialize(self, params: InitializeParams) -> InitializeResult:
        ...
        did_close = did_close_diagnostics if diagnostics.enable else did_close_default
...
def completion_item_resolve(
    ...
    return jedi_utils.lsp_completion_item_resolve(params, markup_kind=markup_kind)
...
def highlight(
    ...
    highlight_names = [
        DocumentHighlight(range=lsp_range) for lsp_range in lsp_ranges if lsp_range
    ]
...
def rename(server: JediLanguageServer, params: RenameParams) -> Optional[WorkspaceEdit]:
    ...
    try:
        ...
        return None
    changes = text_edit_utils.lsp_document_changes(server.workspace, refactoring)
...
def code_action(
    ...
    extract_var = server.initialization_options.code_action.name_extract_variable
    try:
    ...
    extract_func = server.initialization_options.code_action.name_extract_function
    try:
