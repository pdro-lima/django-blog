                ...
                for if_name in if_names:
                    ...
                    if len(definitions) > 1:
                        if len(name_dicts) * len(definitions) > 16:
                            debug.dbg(
                                "Too many options for if branch inference %s.", if_stmt
                            )
    ...
    for value in value_set:
        if operator == "-":
            ...
            if b is None:  # Uncertainty.
                yield list(
                    value.inference_state.builtins_module.py__getattribute__(
                        "bool"
                    ).execute_annotation()
                ).pop()
            else:
...
def _infer_comparison_part(inference_state, context, left, operator, right):
    ...
    if str_operator == "*":
        ...
        return inference_state.builtins_module.py__getattribute__(
            "bool"
        ).execute_annotation()
...
def _infer_subscript_list(context, index):
    ...
    if index == ":":
        ...
        return ValueSet(
            [iterable.SequenceLiteralValue(context.inference_state, context, index)]
        )
