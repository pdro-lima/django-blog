...
def django_discovery_runner(manage_py_path: str, args: List[str]) -> None:
    ...
    if not pathlib.Path(manage_py_path).exists():
        raise VSCodeUnittestError(
            "Error running Django, manage.py path does not exist."
        )
    ...
    try:
        ...
        if "PYTHONPATH" in env:
            env["PYTHONPATH"] = (
                os.fspath(custom_test_runner_dir) + os.pathsep + env["PYTHONPATH"]
            )
        else:
...
def django_execution_runner(
    manage_py_path: str, test_ids: List[str], args: List[str]
) -> None:
    # Attempt a small amount of validation on the manage.py path.
    if not pathlib.Path(manage_py_path).exists():
        raise VSCodeUnittestError(
