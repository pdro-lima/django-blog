...
def get_text_list_for_frame(frame):
    ...
    try:
        while curFrame:
            ...
            absolute_filename = (
                pydevd_file_utils.get_abs_path_real_path_and_base_from_frame(curFrame)[
                    0
                ]
            )
            ...
            cmdTextList.append(
                'file="%s" line="%s">' % (quote(my_file, "/>_= \t"), myLine)
            )
...
def send_concurrency_message(
    ...
    if type == "lock":
        cmdTextList.append(
            ' lock_id="%s"' % pydevd_xml.make_valid_xml_value(str(lock_id))
        )
...
class ThreadingLogger:
    ...
    def log_event(self, frame):
        ...
        if "self" in frame.f_locals:
            ...
            if (
                isinstance(self_obj, threading.Thread)
                or self_obj.__class__ == ObjectWrapper
            ):
                write_log = True
        ...
        try:
            if write_log:
                ...
                if not back:
                    return
                name, _, back_base = (
                    pydevd_file_utils.get_abs_path_real_path_and_base_from_frame(back)
                )
                ...
                if isinstance(self_obj, threading.Thread):
                    ...
                    if (method_name in THREAD_METHODS) and (
                        back_base not in DONT_TRACE_THREADING
                        or (method_name in INNER_METHODS and back_base in INNER_FILES)
                    ):
                        ...
                        if real_method == "_stop":
                            if (
                                back_base in INNER_FILES
                                and back.f_code.co_name == "_wait_for_tstate_lock"
                            ):
                ...
                if self_obj.__class__ == ObjectWrapper:
                    ...
                    if "attr" in frame.f_locals and (
                        ...
                        or frame.f_locals["attr"] in QUEUE_METHODS
                    ):
