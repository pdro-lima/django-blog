...
class TryExceptInfo(object):
    ...
    def __str__(self):
        ...
        if self.raise_lines_in_except:
            lst.append(
                " raises: %s" % (", ".join(str(x) for x in self.raise_lines_in_except),)
            )
...
def _get_line(op_offset_to_line, op_offset, firstlineno, search=False):
    ...
    raise AssertionError(
        "Unable to find line for offset: %s.Info: %s"
...
_Instruction = namedtuple(
    "_Instruction",
    "opname, opcode, starts_line, argval, is_jump_target, offset, argrepr",
)
    ...
    for instruction in iter_instructions(co):
        ...
        if curr_op_name in ("RETURN_VALUE", "RETURN_CONST"):
            lst.append(
                ReturnInfo(
                    _get_line(
                        op_offset_to_line, instruction.offset, firstlineno, search=True
                    )
                )
            )
...
if sys.version_info[:2] <= (3, 9):
    ...
    def _get_except_target_info(
        ...
        next_3 = [
            ...
            for j_instruction in instructions[
                exception_end_instruction_index : exception_end_instruction_index + 3
            ]
        ]
        ...
        if next_3 == [
            ...
            "POP_TOP",
            "POP_TOP",
        ]:  # try..except without checking exception.
            try:
            ...
            for j, jump_if_not_exc_instruction in enumerate(iter_in):
                if jump_if_not_exc_instruction.opname == "JUMP_IF_NOT_EXC_MATCH":
                    # Python 3.9
                    except_end_instruction = instructions[
                        offset_to_instruction_idx[jump_if_not_exc_instruction.argval]
                    ]
                    return _TargetInfo(
                        except_end_instruction, jump_if_not_exc_instruction
                    )

                elif (
                    jump_if_not_exc_instruction.opname == "COMPARE_OP"
                    and jump_if_not_exc_instruction.argval == "exception match"
                ):
    ...
    def collect_try_except_info(co, use_func_first_line=False):
        ...
        op_offset_to_line = dict(
            entry for entry in dis.findlinestarts(co) if entry[1] is not None
        )
        ...
        for i, instruction in enumerate(instructions):
            ...
            if curr_op_name in (
                "SETUP_FINALLY",
                "SETUP_EXCEPT",
            ):  # SETUP_EXCEPT before Python 3.8, SETUP_FINALLY Python 3.8 onwards.
                exception_end_instruction_index = offset_to_instruction_idx[
                    instruction.argval
                ]
                ...
                target_info = _get_except_target_info(
                    instructions,
                ...
                while target_info is not None:
                    ...
                    if jump_instruction is not None:
                        ...
                        target_info = _get_except_target_info(
                            instructions, check_index, offset_to_instruction_idx
                        )
                    else:
                        break
                ...
                if except_end_instruction is not None:
                    ...
                    try_except_info.except_end_bytecode_offset = (
                        except_end_instruction.offset
                    )
                    try_except_info.except_end_line = _get_line(
                        op_offset_to_line,
                        except_end_instruction.offset,
                        firstlineno,
                        search=True,
                    ...
                    for raise_instruction in instructions[
                        i : offset_to_instruction_idx[
                            try_except_info.except_end_bytecode_offset
                        ]
                    ]:
    ...
    def _get_except_target_info(
        ...
        if next_3 == [
            ...
            for pop_except_instruction in instructions[
                exception_end_instruction_index + 3 :
            ]:
                if pop_except_instruction.opname == "POP_EXCEPT":
                    except_end_instruction = pop_except_instruction
                    return _TargetInfo(except_end_instruction)
        ...
        for i, instruction in enumerate(instructions):
            ...
            if curr_op_name == "SETUP_FINALLY":
                ...
                if except_end_instruction is not None:
                    ...
                    except_end_line = -1
                    start_i = offset_to_instruction_idx[
                        try_except_info.except_bytecode_offset
                    ]
                    ...
                    for instruction in instructions[start_i : end_i + 1]:
                        ...
                        if (
                            found_at_line is not None
                            and found_at_line > except_end_line
                        ):
...
class _Disassembler(object):
    def __init__(self, co, firstlineno, level=0):
        ...
        op_offset_to_line = self.op_offset_to_line = dict(
            entry for entry in dis.findlinestarts(co) if entry[1] is not None
        )
    ...
    def _lookahead(self):
        ...
        if next_instruction.opname in ("CALL_FUNCTION", "PRECALL", "CALL"):
            ...
            del self.instructions[
                delta : delta + next_instruction.argval + 1
            ]  # +1 = BUILD_TUPLE
            ...
            if self.instructions[0].opname == "RETURN_VALUE":
                ...
                return (
                    msg(
                        next_instruction,
                        "return ",
                        line=self.min_line(next_instruction, fullrepr),
                    ),
                    fullrepr,
                )
    ...
    def _next_instruction_to_str(self, line_to_contents):
        ...
        if instruction.opname == "RETURN_CONST":
            return (
                msg(instruction, "return ", line=self.min_line(instruction)),
                msg(instruction),
            )
        ...
        if instruction.opname in (
            ...
            if (
                next_instruction.opname == "RAISE_VARARGS"
                and next_instruction.argval == 1
            ):
                self.instructions.pop(0)
                return (
                    msg(instruction, "raise ", line=self.min_line(instruction)),
                    msg(instruction),
                )
