...
class _MakeFunctionPy3(_BaseHandler):
    ...
    def _handle(self):
        ...
        is_lambda = self.is_lambda = "<lambda>" in [
            x.tok for x in self.qualified_name.tokens
        ]
        ...
        if default_node:
            defaults = (
                [_SENTINEL]
                * (len(code.co_varnames) - len(default_node.instruction.argval))
            ) + list(default_node.instruction.argval)
        else:
        ...
        for i, arg in enumerate(code.co_varnames):
            ...
            if default is not _SENTINEL:
                ...
                default_token = _Token(
                    default_node.i_line, None, str(default), after=prev
                )
...
def _compose_line_contents(line_contents, previous_line_tokens):
    ...
    while line_contents:
        ...
        if not added:
            ...
            pydev_log.critical(
                "Error. After markers are not correct:\n%s", stream.getvalue()
            )
            break
    return "".join(lst)
