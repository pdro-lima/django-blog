...
class _PyDevCommandProcessor(object):
    ...
    def _cmd_set_next(self, py_db, cmd_id, seq, text):
        ...
        return self.api.request_set_next(
            py_db, seq, thread_id, cmd_id, None, line, func_name
        )
    ...
    def cmd_smart_step_into(self, py_db, cmd_id, seq, text):
        ...
        if line_or_bytecode_offset.startswith("offset="):
            ...
            if ";" in temp:
                ...
                child_offset = -1
                offset = int(temp)
            return self.api.request_smart_step_into(
                py_db, seq, thread_id, offset, child_offset
            )
        else:
            # If the offset wasn't passed, just use the line/func_name to do the stop.
            return self.api.request_smart_step_into_by_func_name(
                py_db, seq, thread_id, line_or_bytecode_offset, func_name
            )
    ...
    def cmd_change_variable(self, py_db, cmd_id, seq, text):
        ...
        self.api.request_change_variable(
            py_db, seq, thread_id, frame_id, scope, attr, value
        )
    ...
    def cmd_get_array(self, py_db, cmd_id, seq, text):
        ...
        roffset, coffset, rows, cols, format, thread_id, frame_id, scopeattrs = (
            text.split("\t", 7)
        )
    ...
    def cmd_set_break(self, py_db, cmd_id, seq, text):
        ...
        if py_db._set_breakpoints_with_id:
            ...
            condition = (
                condition.replace("@_@NEW_LINE_CHAR@_@", "\n")
                .replace("@_@TAB_CHAR@_@", "\t")
                .strip()
            )

            expression = (
                expression.replace("@_@NEW_LINE_CHAR@_@", "\n")
                .replace("@_@TAB_CHAR@_@", "\t")
                .strip()
            )
        else:
            ...
            btype, filename, line, func_name, suspend_policy, condition, expression = (
                text.split("\t", 6)
            )
        ...
        if hit_condition is not None and (
            len(hit_condition) <= 0 or hit_condition == "None"
        ):
            hit_condition = None
        ...
        def on_changed_breakpoint_state(breakpoint_id, add_breakpoint_result):
            ...
            if error_code:
                if error_code == self.api.ADD_BREAKPOINT_FILE_NOT_FOUND:
                    msg = (
                        "pydev debugger: Trying to add breakpoint to file that does not exist: %s (will have no effect).\n"
                        % (translated_filename,)
                    )

                elif error_code == self.api.ADD_BREAKPOINT_FILE_EXCLUDED_BY_FILTERS:
                    msg = (
                        "pydev debugger: Trying to add breakpoint to file that is excluded by filters: %s (will have no effect).\n"
                        % (translated_filename,)
                    )
                    ...
                    msg = (
                        "pydev debugger: Trying to add breakpoint to line (%s) that is not valid in: %s.\n"
                        % (
                            translated_line,
                            translated_filename,
                        )
                    ...
                    # Shouldn't get here.
                    msg = (
                        "pydev debugger: Breakpoint not validated (reason unknown -- please report as error): %s (%s).\n"
                        % (
                            translated_filename,
                            translated_line,
                        )
                ...
                if add_breakpoint_result.original_line != translated_line:
                    msg = (
                        "pydev debugger (info): Breakpoint in line: %s moved to line: %s (in %s).\n"
                        % (
    ...
    def _cmd_exec_or_evaluate_expression(self, py_db, cmd_id, seq, text):
        ...
        attr_to_set_result = ""
        try:
            thread_id, frame_id, scope, expression, trim, attr_to_set_result = (
                text.split("\t", 5)
            )
        except ValueError:
    ...
    def cmd_set_py_exception_json(self, py_db, cmd_id, seq, text):
        ...
        try:
            ...
            py_db.skip_on_exceptions_thrown_in_same_context = as_json.get(
    ...
    def cmd_set_py_exception(self, py_db, cmd_id, seq, text):
        ...
        try:
            ...
            if len(splitted) >= 5:
                ...
                pydev_log.exception(
                    "Expected to have at least 5 ';' separated items. Received: %s",
                    text,
                )
    ...
    def cmd_add_django_exception_break(self, py_db, cmd_id, seq, text):
        self.api.add_plugins_exception_breakpoint(
            py_db, breakpoint_type="django", exception=text
        )

    def cmd_remove_django_exception_break(self, py_db, cmd_id, seq, text):
        self.api.remove_plugins_exception_breakpoint(
            py_db, exception_type="django", exception=text
        )
            ...
            if text:
                for line in text.split("||"):  # Can be bulk-created (one in each line)
                    ...
                    canonical_normalized_filename = (
                        pydevd_file_utils.canonical_normalized_path(original_filename)
                    )
                    absolute_filename = pydevd_file_utils.absolute_path(
                        original_filename
                    )
                    ...
                    if os.path.exists(absolute_filename):
                        ...
                        if lines_ignored is None:
                            lines_ignored = (
                                py_db.filename_to_lines_where_exceptions_are_ignored[
                                    canonical_normalized_filename
                                ]
                            ) = ({})
    ...
    def cmd_enable_dont_trace(self, py_db, cmd_id, seq, text):
        if text:
            true_str = (
                "true"  # Not all 3.x versions support u'str', so, doing workaround.
            )
