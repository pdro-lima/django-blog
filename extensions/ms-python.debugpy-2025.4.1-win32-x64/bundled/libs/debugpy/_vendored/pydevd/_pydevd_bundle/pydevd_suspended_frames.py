...
from _pydevd_bundle.pydevd_resolver import (
...
class _AbstractVariable(object):
    ...
    def get_var_data(
        ...
        **safe_repr_custom_attrs
        ...
        type_name, _type_qualifier, _is_exception_on_eval, resolver, value = (
            get_variable_details(self.value, to_string=safe_repr, context=context)
        )
        ...
        if resolver is not None:  # I.e.: it's a container
            ...
            var_data["variablesReference"] = (
                0  # It's mandatory (although if == 0 it doesn't have children).
            )
        ...
        if resolver is not None:  # i.e.: it's a container.
            ...
            if parent_evaluate_name:
                ...
                for key, val, evaluate_name in lst:
                    # No evaluate name
                    variable = _ObjectVariable(
                        self.py_db, key, val, self._register_variable, frame=self.frame
                    )
                    children_variables.append(variable)
    ...
    def get_children_variables(self, fmt=None, scope=None):
        ...
        lst, group_entries = self._group_entries(
            [
                ...
                if x[0] != "_pydev_stop_at_break"
            ],
            handle_return_values=True,
        )
        group_variables = []

        for key, val, _ in group_entries:
            # Make sure that the contents in the group are also sorted.
            val.contents_debug_adapter_protocol.sort(
                key=lambda v: sorted_attributes_key(v[0])
            )
            variable = _ObjectVariable(
                self.py_db,
                key,
...
class _FramesTracker(object):
    ...
    def track(self, thread_id, frames_list, frame_custom_thread_id=None):
        ...
        with self._lock:
            ...
            if (
                ...
                in self._suspended_frames_manager._thread_id_to_tracker
            ):
                sys.stderr.write(
                    "pydevd: Something is wrong. Tracker being added twice to the same thread id.\n"
                )
            ...
            for frame in frames_list:
                ...
                _FrameVariable(
                    self.py_db, frame, self._register_variable
                )  # Instancing is enough to register.
                self._suspended_frames_manager._variable_reference_to_frames_tracker[
                    frame_id
                ] = self
    ...
    def untrack_all(self):
        with self._lock:
            ...
            for thread_id in self._thread_id_to_frame_ids:
                self._suspended_frames_manager._thread_id_to_tracker.pop(
                    thread_id, None
                )

            for frame_id in self._frame_id_to_frame:
                del self._suspended_frames_manager._variable_reference_to_frames_tracker[
                    frame_id
                ]
    ...
    def create_thread_suspend_command(
        self,
        thread_id,
        stop_reason,
        message,
