...
class Frame(object):
    def __init__(
        self, f_back, f_fileno, f_code, f_locals, f_globals=None, f_trace=None
    ):
        self.f_back = f_back
    ...
    def map_columns_to_line(self, original_line: str):
        ...
        if self.lineno == self.end_lineno:
            try:
                ...
                if ret is not None:
                    return (
                        _utf8_byte_offset_to_character_offset(
                            original_line, ret[0] + self.colno
                        ),
                        _utf8_byte_offset_to_character_offset(
                            original_line, ret[1] + self.colno
                        ),
                    )
            except Exception:
...
def _extract_caret_anchors_in_bytes_from_line_segment(segment: str):
    ...
    if isinstance(statement, ast.Expr):
        ...
        if isinstance(expr, ast.BinOp):
            ...
            if operator_offset + 1 < len(operator_str) and not operator_str[
                operator_offset + 1
            ] == ord(b" "):
                right_anchor += 1
...
class _DummyFrameWrapper(object):
    ...
    def __str__(self):
        return "<_DummyFrameWrapper, file '%s', line %s, %s" % (
...
def create_frames_list_from_exception_cause(trace_obj, frame, exc_type, exc_desc, memo):
    ...
    while tb is not None:
        ...
        lst.append(
            (
                _DummyFrameWrapper(tb.tb_frame, tb.tb_lineno, None),
                tb.tb_lineno,
                _get_line_col_info_from_tb(tb),
            )
        )
        tb = tb.tb_next
...
def create_frames_list_from_traceback(
    trace_obj, frame, exc_type, exc_desc, exception_type=None
):
    """
    :param trace_obj:
    ...
    for tb_frame, tb_lineno, line_col_info in reversed(lst):
        if frames_list is None and (
            (frame is tb_frame)
            or (frame is None)
            or (exception_type == EXCEPTION_TYPE_USER_UNHANDLED)
        ):
            frames_list = FramesList()
    ...
    while True:
        chained = create_frames_list_from_exception_cause(
            None, None, None, curr.exc_desc, memo
        )
        if chained is None:
            break
        else:
