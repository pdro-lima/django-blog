...
from _pydev_bundle._pydev_imports_tipper import (
    TYPE_IMPORT,
    TYPE_CLASS,
    TYPE_FUNCTION,
    TYPE_ATTR,
    ...
    def track_module(self, filename_in_utf8, module_name, frame):
        ...
        with self._lock:
            ...
            module_events.append(
                NetCommand(CMD_MODULE_EVENT, 0, module_event, is_json=True)
            )
...
class NetCommandFactoryJson(NetCommandFactory):
        ...
        for completion in completions:
            ...
            if label.lower().startswith(qualifier):
                completion = pydevd_schema.CompletionItem(
                    label=label,
                    type=COMPLETION_TYPE_LOOK_UP[completion[3]],
                    start=start,
                    length=qualifier_len,
                )
    ...
    def _format_frame_name(self, fmt, initial_name, module_name, line, path):
        ...
        if fmt.get("module", False):
            if module_name:
                ...
                basename = (
                    basename[0:-3] if basename.lower().endswith(".py") else basename
                )
                if initial_name == "<module>":
    ...
    def make_get_thread_stack_message(
        ...
        try:
            ...
            for (
                ...
                source_reference = (
                    pydevd_file_utils.get_client_filename_source_reference(
                        filename_in_utf8
                    )
                )
                ...
                if (
                    ...
                    and not applied_mapping
                    and not os.path.exists(original_filename)
                ):
                    if getattr(frame.f_code, "co_lines", None) or getattr(
                        frame.f_code, "co_lnotab", None
                    ):
                        ...
                        source_reference = (
                            pydevd_file_utils.create_source_reference_for_frame_id(
                        ...
                        if linecache.getline(original_filename, 1):
                            source_reference = (
                                pydevd_file_utils.create_source_reference_for_linecache(
                                    original_filename
                                )
                            )
                ...
                if line_col_info is not None:
                    try:
                        ...
                        if DebugInfoHolder.DEBUG_TRACE_LEVEL >= 2:
                            pydev_log.exception(
                                "Unable to get line from linecache for file: %s",
                                original_filename,
                            )
                    else:
                        if line_text:
                            colno, endcolno = line_col_info.map_columns_to_line(
                                line_text
                            )
                            column = colno + 1
    ...
    def make_thread_suspend_single_notification(
        ...
        return NetCommand(
            CMD_THREAD_SUSPEND_SINGLE_NOTIFICATION, 0, event, is_json=True
        )
    ...
    def make_thread_suspend_message(
        ...
        pydev_log.debug(
            "Sending per-thread thread suspend notification (stop_reason: %s)",
            stop_reason,
        )
