...
def getVariable(dbg, thread_id, frame_id, scope, locator):
    ...
    if scope == "EXPRESSION":
        ...
        if scope == "GLOBAL":
            ...
            del locator_parts[
                0
            ]  # globals are special, and they get a single dummy unused attribute
        else:
...
def custom_operation(
    dbg, thread_id, frame_id, scope, attrs, style, code_or_file, operation_fn_name
):
    """
    ...
    try:
        ...
        try:
            if ".__" in expression:
                ...
                curr = local_vars[
                    entry
                ]  # Note: we want the KeyError if it's not there.
...
def _run_with_interrupt_thread(
    original_func, py_db, curr_thread, frame, expression, is_exec
):
    ...
    if interrupt_thread_timeout > 0:
        ...
        pydev_log.info(
            "Doing evaluate with interrupt threads timeout: %s.",
    ...
    if on_interrupt_threads is None:
        ...
        with timeout_tracker.call_on_timeout(
            interrupt_thread_timeout, on_interrupt_threads
        ):
            return original_func(py_db, frame, expression, is_exec)
...
def _run_with_unblock_threads(
    ...
    if py_db.multi_threads_single_notification:
        ...
        unblock_threads_timeout = (
            -1
        )  # Don't use this if threads are managed individually.

    if unblock_threads_timeout >= 0:
        pydev_log.info(
            "Doing evaluate with unblock threads timeout: %s.", unblock_threads_timeout
        )
        ...
        def on_timeout_unblock_threads():
            ...
            py_db.threads_suspended_single_notification.on_thread_resume(
                tid, curr_thread
            )
    ...
    try:
        if on_timeout_unblock_threads is None:
            return _run_with_interrupt_thread(
                original_func, py_db, curr_thread, frame, expression, is_exec
            )
        else:
            with timeout_tracker.call_on_timeout(
        ...
        if on_timeout_unblock_threads is not None and on_timeout_unblock_threads.called:
            ...
            py_db.threads_suspended_single_notification.on_thread_suspend(
                tid, curr_thread, CMD_SET_BREAK
            )
...
def _evaluate_with_timeouts(original_func):
    ...
    def new_func(py_db, frame, expression, is_exec):
        ...
        timeout_tracker = (
            py_db.timeout_tracker
        )  # : :type timeout_tracker: TimeoutTracker
        with timeout_tracker.call_on_timeout(
            warn_evaluation_timeout, on_warn_evaluation_timeout
        ):
            return _run_with_unblock_threads(
                original_func, py_db, curr_thread, frame, expression, is_exec
            )
        ...
        if is_exec:
            ...
            if compiled is None:
                try:
                    ...
                    is_async = (
                        inspect.CO_COROUTINE & compiled.co_flags == inspect.CO_COROUTINE
                    )
                    if is_async:
                        t = _EvalAwaitInNewEventLoop(
                            py_db, compiled, updated_globals, updated_locals
                        )
                        t.start()
                        t.join()
...
def change_attr_expression(frame, attr, expression, dbg, value=SENTINEL_VALUE):
    ...
    try:
        ...
        if attr[:7] == "Globals":
            ...
            if (
                "." not in attr
            ):  # i.e.: if we have a '.', we're changing some attribute of a local var.
...
def array_to_meta_xml(array, name, format):
    ...
    xml = (
        '<array slice="%s" rows="%s" cols="%s" format="%s" type="%s" max="%s" min="%s"/>'
        % (
            slice,
            rows,
            cols,
...
def dataframe_to_xml(df, name, roffset, coffset, rows, cols, format):
    ...
    slice = name + slice
    xml = (
        '<array slice="%s" rows="%s" cols="%s" format="" type="" max="0" min="0"/>\n'
    ...
    get_label = lambda label: (
        str(label) if not isinstance(label, tuple) else "/".join(map(str, label))
    )
    ...
    for col in range(cols):
        ...
        xml += (
            '<colheader index="%s" label="%s" type="%s" format="%s" max="%s" min="%s" />\n'
            % (
                str(col),
