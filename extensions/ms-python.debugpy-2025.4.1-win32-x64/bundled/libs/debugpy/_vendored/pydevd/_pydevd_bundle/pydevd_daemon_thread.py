...
class PyDBDaemonThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            try:
                if IS_JYTHON and not isinstance(
...
def _patch_threading_to_hide_pydevd_threads():
    ...
    if new_threading_enumerate is not None:
        ...
        _pydev_saved_modules.pydevd_saved_threading_enumerate = (
            pydevd_saved_threading_enumerate
        )
        ...
        if hasattr(threading, "_pickSomeNonDaemonThread"):
            ...
            threading._pickSomeNonDaemonThread.__code__ = (
                new_pick_some_non_daemon_thread.__code__
            )
...
def mark_as_pydevd_daemon_thread(thread):
    if (
        not IS_JYTHON
        and not IS_IRONPYTHON
        and PYDEVD_APPLY_PATCHING_TO_HIDE_PYDEVD_THREADS
    ):
