...
from _pydevd_bundle.pydevd_json_debug_options import (
    _extract_debug_options,
    DebugOptions,
)
    ...
    if not isinstance(rules, list):
        ...
        for rule in rules:
            if not isinstance(rule, dict):
                on_error(
                    'Invalid "rules" (expected list of dicts). Found: %s' % (rules,)
                )
                continue
            ...
            if include is None:
                on_error(
                    'Invalid "rule" (expected dict with "include"). Found: %s' % (rule,)
                )
                continue
            ...
            if path is None and module is None:
                on_error(
                    'Invalid "rule" (expected dict with "path" or "module"). Found: %s'
                    % (rule,)
                )
                continue
                ...
                if "*" not in path and "?" not in path:
                    if os.path.isdir(glob_pattern):
                        ...
                        if not glob_pattern.endswith("/") and not glob_pattern.endswith(
                            ...
                            glob_pattern += "/"
                        glob_pattern += "**"
                    directory_exclude_filters.append(
                        ExcludeFilter(glob_pattern, not include, True)
                    )
                else:
                    glob_exclude_filters.append(
                        ExcludeFilter(glob_pattern, not include, True)
                    )
        ...
        directory_exclude_filters = sorted(
            directory_exclude_filters,
            key=lambda exclude_filter: -len(exclude_filter.name),
        )
        module_exclude_filters = sorted(
            module_exclude_filters, key=lambda exclude_filter: -len(exclude_filter.name)
        )
        exclude_filters = (
            directory_exclude_filters + glob_exclude_filters + module_exclude_filters
        )
...
class PyDevJsonCommandProcessor(object):
    ...
    def process_net_command_json(self, py_db, json_contents, send_response=True):
        ...
        try:
            ...
            try:
                ...
                request = Request(
                    loaded_json.get("command", "<unknown>"), loaded_json["seq"]
                )
            except:
            ...
            if DebugInfoHolder.DEBUG_TRACE_LEVEL >= 1:
                pydev_log.info(
                    ...
                    % (
                        ...
                        json.dumps(
                            request.to_dict(update_ids_to_dap=True),
                            indent=4,
                            sort_keys=True,
                        ),
                    )
        ...
        with py_db._main_lock:
            if request.__class__ == PydevdAuthorizeRequest:
                authorize_request = (
                    request  # : :type authorize_request: PydevdAuthorizeRequest
                )
    ...
    def on_completions_request(self, py_db, request):
        ...
        self.api.request_completions(
            py_db, seq, thread_id, frame_id, text, line=line, column=column
        )
    ...
    def _set_debug_options(self, py_db, args, start_reason):
        ...
        terminate_keyboard_interrupt = (
            args.get("onTerminate", "kill") == "KeyboardInterrupt"
        )
        ...
        if not isinstance(auto_reload, dict):
            pydev_log.info(
                "Expected autoReload to be a dict. Received: %s" % (auto_reload,)
            )
            auto_reload = {}
        ...
        if not watch_dirs:
            ...
            if argv:
                ...
                if (
                    f
                ):  # argv[0] could be None (https://github.com/microsoft/debugpy/issues/987)
                    if os.path.isdir(f):
                        watch_dirs.append(f)
                    else:
        ...
        for w in watch_dirs:
            try:
                new_watch_dirs.add(
                    pydevd_file_utils.get_path_with_real_case(
                        pydevd_file_utils.absolute_path(w)
                    )
                )
            except Exception:
    ...
    def _handle_launch_or_attach_request(self, py_db, request, start_reason):
        ...
        self._set_debug_options(
            py_db, request.arguments.kwargs, start_reason=start_reason
        )
        response = pydevd_base_schema.build_response(request)

        initialized_event = InitializedEvent()
        py_db.writer.add_command(
            NetCommand(CMD_RETURN, 0, initialized_event, is_json=True)
        )
        return NetCommand(CMD_RETURN, 0, response, is_json=True)
    ...
    def on_launch_request(self, py_db, request):
        ...
        """
        return self._handle_launch_or_attach_request(
            py_db, request, start_reason="launch"
        )
    ...
    def on_attach_request(self, py_db, request):
        ...
        """
        return self._handle_launch_or_attach_request(
            py_db, request, start_reason="attach"
        )
        ...
        if target_id is not None:
            ...
            if variant is not None:
                ...
                if parent is not None:
                    self.api.request_smart_step_into(
                        py_db, request.seq, thread_id, parent.offset, variant.offset
                    )
                else:
                    self.api.request_smart_step_into(
                        py_db, request.seq, thread_id, variant.offset, -1
                    )
            else:
                variables_response = pydevd_base_schema.build_response(
                    request,
    ...
    def on_stepintargets_request(self, py_db, request):
        ...
        py_db.post_method_as_internal_command(
            thread_id,
            internal_get_step_in_targets_json,
            request.seq,
            thread_id,
            frame_id,
            request,
        ...
        for bp in arguments.breakpoints:
            ...
            breakpoints_set.append(
                pydevd_schema.Breakpoint(
                    verified=True, id=self._next_breakpoint_id()
                ).to_dict()
            )
    ...
    def on_setbreakpoints_request(self, py_db, request):
        ...
        if not filename.lower().endswith(
            ".py"
        ):  # Note: check based on original file, not mapping.
        ...
        for source_breakpoint in arguments.breakpoints:
            ...
            on_changed_breakpoint_state = partial(
                self._on_changed_breakpoint_state, py_db, arguments.source
            )
            ...
            bp = self._create_breakpoint_from_add_breakpoint_result(
                py_db, arguments.source, breakpoint_id, result
            )
    ...
    def _on_changed_breakpoint_state(self, py_db, source, breakpoint_id, result):
        bp = self._create_breakpoint_from_add_breakpoint_result(
            py_db, source, breakpoint_id, result
        )
    ...
    def _create_breakpoint_from_add_breakpoint_result(
        ...
        if error_code:
            if error_code == self.api.ADD_BREAKPOINT_FILE_NOT_FOUND:
                ...
                # Shouldn't get here.
                error_msg = (
                    "Breakpoint not validated (reason unknown -- please report as bug)."
                )
        ...
        if exception_options:
            ...
            for option in exception_options:
                ...
                notify_on_unhandled_exceptions = (
                    1 if option.breakMode == "unhandled" else 0
                )
                notify_on_user_unhandled_exceptions = (
                    1 if option.breakMode == "userUnhandled" else 0
                )
    ...
    def on_exceptioninfo_request(self, py_db, request):
        ...
        if thread is not None:
            self.api.request_exception_info_json(
                py_db, request, thread_id, thread, max_frames
            )
        else:
            response = Response(
                request_seq=request.seq,
    ...
    def on_scopes_request(self, py_db, request):
        ...
        scopes = [
            Scope(
                "Locals",
                ScopeRequest(int(variables_reference), "locals"),
                False,
    ...
    def on_evaluate_request(self, py_db, request):
        ...
        if arguments.frameId is None:
            ...
            thread_id = (
                py_db.suspended_frames_manager.get_thread_id_for_variable_reference(
                    arguments.frameId
                )
            )
    ...
    def on_modules_request(self, py_db, request):
        modules_manager = (
            py_db.cmd_factory.modules_manager
    ...
    def on_source_request(self, py_db, request):
        ...
        if source_reference != 0:
            server_filename = (
                pydevd_file_utils.get_server_filename_from_source_reference(
                    source_reference
                )
            )
            ...
            if server_filename:
                ...
                if content is None:
                    ...
                    lines = (
                        linecache.getline(server_filename, i)
            ...
            if content is None:
                frame_id = pydevd_file_utils.get_frame_id_from_source_reference(
                    source_reference
                )
                pydev_log.debug(
                    "Found frame id: %s for source reference: %s",
                    frame_id,
                    source_reference,
                )
                if frame_id is not None:
                    try:
                        content = self.api.get_decompiled_source_from_frame_id(
                            py_db, frame_id
                        )
                    except Exception:
                        pydev_log.exception(
                            "Error getting source for frame id: %s", frame_id
                        )
                        content = None
    ...
    def on_goto_request(self, py_db, request):
        ...
        self.api.request_set_next(
            py_db, request.seq, thread_id, CMD_SET_NEXT_STATEMENT, path, line, "*"
        )
    ...
    def on_setdebuggerproperty_request(self, py_db, request):
        ...
        if (
            args.dontTraceStartPatterns is not None
            and args.dontTraceEndPatterns is not None
        ):
            start_patterns = tuple(args.dontTraceStartPatterns)
            end_patterns = tuple(args.dontTraceEndPatterns)
            self.api.set_dont_trace_start_end_patterns(
                py_db, start_patterns, end_patterns
            )
        ...
        if args.multiThreadsSingleNotification is not None:
            py_db.multi_threads_single_notification = (
                args.multiThreadsSingleNotification
            )
