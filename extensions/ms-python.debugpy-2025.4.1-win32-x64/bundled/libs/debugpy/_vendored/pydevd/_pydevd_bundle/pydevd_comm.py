...
from _pydev_bundle.pydev_monkey import (
...
class ReaderThread(PyDBDaemonThread):
    ...
    def __init__(
        ...
        process_net_command,
        terminate_on_socket_close=True,
    ):
        ...
        self.process_net_command_json = PyDevJsonCommandProcessor(
            self._from_json
        ).process_net_command_json

    def _from_json(self, json_msg, update_ids_from_dap=False):
        return pydevd_base_schema.from_json(
            json_msg, update_ids_from_dap, on_dict_loaded=self._on_dict_loaded
        )
        ...
        try:
            ...
            while True:
                    ...
                    if content_len != -1:
                        ...
                        if line == b"\r\n":
                            ...
                            if len(json_contents) == 0:
                                pydev_log.debug(
                                    "ReaderThread: empty contents received (len(json_contents) == 0)."
                                )
                                self._terminate_on_socket_close()
                                return  # Finished communication.
                ...
                try:
                    ...
                    if (
                        sys is not None and pydev_log_exception is not None
                    ):  # Could happen at interpreter shutdown
...
class FSNotifyThread(PyDBDaemonThread):
    ...
    def _on_run(self):
        try:
            pydev_log.info(
                "Watching directories for code reload:\n---\n%s\n---"
            ...
            while not self._kill_received:
                for change_enum, change_path in self.watcher.iter_changes():
                    ...
                    if change_enum == fsnotify.Change.modified:
                        ...
                        pydev_log.info(
                            "Ignored (add or remove) change in: %s", change_path
                        )
        except:
            pydev_log.exception(
                "Error when waiting for filesystem changes in FSNotifyThread."
            )
            ...
            while True:
                try:
                    try:
                        ...
                        if self._kill_received:
                            pydev_log.debug(
                                "WriterThread: kill_received (sock.shutdown(SHUT_WR))"
                            )
                            try:
...
class InternalThreadCommand(object):
    ...
    def __str__(self):
        return "InternalThreadCommands(%s, %s, %s)" % (
            self.method,
            self.args,
            self.kwargs,
        )
    ...
    try:
        ...
        if not modules_to_reload:
            if filename and module_name:
                _send_io_message(
                    dbg,
                    "code reload: Unable to find module %s to reload for path: %s\n"
                    % (module_name, filename),
                )
            elif filename:
                _send_io_message(
                    dbg,
                    "code reload: Unable to find module to reload for path: %s\n"
                    % (filename,),
                )
            elif module_name:
                _send_io_message(
                    dbg,
                    "code reload: Unable to find module to reload: %s\n"
                    % (module_name,),
                )
            ...
            for module, module_name in modules_to_reload.values():
                _send_io_message(
                    dbg,
                    'code reload: Start reloading module: "' + module_name + '" ... \n',
                )
...
def internal_get_variable_json(py_db, request):
    ...
    py_db.writer.add_command(
        NetCommand(CMD_RETURN, 0, variables_response, is_json=True)
    )
...
class InternalGetVariable(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            keys = list(val_dict)
            if not (
                type_name == "OrderedDict"
                or val_dict.__class__.__name__ == "OrderedDict"
                or IS_PY36_OR_GREATER
            ):
            ...
            xml.write("</xml>")
            cmd = dbg.cmd_factory.make_get_variable_message(
                self.sequence, xml.getvalue()
            )
            xml.close()
            dbg.writer.add_command(cmd)
        except Exception:
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence,
                "Error resolving variables %s" % (get_exception_traceback_str(),),
            )
            dbg.writer.add_command(cmd)
...
class InternalGetArray(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            var = pydevd_vars.eval_in_context(
                self.name, frame.f_globals, frame.f_locals, py_db=dbg
            )
            xml = pydevd_vars.table_like_struct_to_xml(
                var,
                self.name,
                self.roffset,
                self.coffset,
            ...
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error resolving array: " + get_exception_traceback_str()
            )
            dbg.writer.add_command(cmd)
...
def internal_change_variable(dbg, seq, thread_id, frame_id, scope, attr, value):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq,
            "Error changing variable attr:%s expression:%s traceback:%s"
            % (attr, value, get_exception_traceback_str()),
        )
        dbg.writer.add_command(cmd)
...
def internal_change_variable_json(py_db, request):
    ...
    if variable is None:
        _write_variable_response(
            ...
            value="",
            success=False,
            message="Unable to find variable container to change: %s."
            % (variables_reference,),
        )
        return

    child_var = variable.change_variable(
        arguments.name, arguments.value, py_db, fmt=fmt
    )
...
def _write_variable_response(py_db, request, value, success, message):
    body = SetVariableResponseBody("")
    variables_response = pydevd_base_schema.build_response(
        request, kwargs={"body": body, "success": False, "message": message}
    )
...
def internal_get_frame(dbg, seq, thread_id, frame_id):
    ...
    try:
        ...
        if frame is not None:
            ...
            cmd = dbg.cmd_factory.make_error_message(
                seq, "Frame not found: %s from thread: %s" % (frame_id, thread_id)
            )
            dbg.writer.add_command(cmd)
    except:
        cmd = dbg.cmd_factory.make_error_message(
            seq, "Error resolving frame: %s from thread: %s" % (frame_id, thread_id)
        )
        dbg.writer.add_command(cmd)


def internal_get_smart_step_into_variants(
    dbg, seq, thread_id, frame_id, start_line, end_line, set_additional_thread_info
):
    try:
        ...
        if pydevd_bytecode_utils is None:
            ...
            variants = pydevd_bytecode_utils.calculate_smart_step_into_variants(
                frame, int(start_line), int(end_line)
            )
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq,
            "Error getting smart step into variants for frame: %s from thread: %s"
            % (frame_id, thread_id),
        )
        dbg.writer.add_command(cmd)


def internal_get_step_in_targets_json(
    dbg, seq, thread_id, frame_id, request, set_additional_thread_info
):
    try:
        thread = pydevd_find_thread_by_id(thread_id)
        frame = dbg.find_frame(thread_id, frame_id)
                        ...
                        if child_variant.call_order > 1:
                            targets.append(
                                StepInTarget(
                                    id=target_id,
                                    label="%s (call %s)"
                                    % (child_variant.name, child_variant.call_order),
                                )
                            )
                        else:
                            targets.append(
                                StepInTarget(id=target_id, label=child_variant.name)
                            )
                    ...
                    if variant.call_order > 1:
                        targets.append(
                            StepInTarget(
                                id=target_id,
                                label="%s (call %s)"
                                % (variant.name, variant.call_order),
                            )
                        )
                    else:
        ...
        body = StepInTargetsResponseBody([])
        variables_response = pydevd_base_schema.build_response(
            request, kwargs={"body": body, "success": False, "message": str(e)}
        )
        cmd = NetCommand(CMD_RETURN, 0, variables_response, is_json=True)
        dbg.writer.add_command(cmd)
...
def _evaluate_response(py_db, request, result, error_message=""):
    ...
    if not error_message:
        ...
        body = pydevd_schema.EvaluateResponseBody(result=result, variablesReference=0)
        variables_response = pydevd_base_schema.build_response(
            request, kwargs={"body": body, "success": False, "message": error_message}
        )
        py_db.writer.add_command(
            NetCommand(CMD_RETURN, 0, variables_response, is_json=True)
        )
...
def internal_evaluate_expression_json(py_db, request, thread_id):
    ...
    with ctx:
        ...
        if frame_id is None:
            ...
            eval_result = pydevd_vars.evaluate_expression(
                py_db, frame, expression, is_exec=False
            )
            is_error = isinstance_checked(eval_result, ExceptionOnEvaluate)
            if is_error:
                if (
                    context == "hover"
                ):  # In a hover it doesn't make sense to do an exec.
                    _evaluate_response(
                        py_db,
                        request,
                        result="",
                        error_message="Exception occurred during evaluation.",
                    )
                    return
                elif context == "watch":
                    ...
                    try:
                        ...
                        if context == "repl":
                            ...
                            _evaluate_response_return_exception(
                                py_db,
                                request,
                                eval_result.etype,
                                eval_result.result,
        ...
        if frame_tracker is None:
            ...
            _evaluate_response(
                ...
                request,
                result="",
                error_message="Thread id: %s is not current thread id." % (thread_id,),
            )
            return
        ...
        if context == "repl" and eval_result is None:
            ...
            variable = frame_tracker.obtain_as_variable(
                expression, eval_result, frame=frame
            )
            var_data = variable.get_var_data(
                fmt=fmt, context=context, **safe_repr_custom_attrs
            )
...
def internal_evaluate_expression(
    ...
    try:
        ...
        if frame is not None:
            ...
            if attr_to_set_result != "":
                pydevd_vars.change_attr_expression(
                    frame, attr_to_set_result, expression, dbg, result
                )
        else:
            result = None
        ...
        cmd = dbg.cmd_factory.make_error_message(
            seq, "Error evaluating expression " + exc
        )
        dbg.writer.add_command(cmd)
...
def internal_set_expression_json(py_db, request, thread_id):
    ...
    try:
        ...
        _set_expression_response(
            py_db, request, error_message="Error executing: %s" % (exec_code,)
        )
        return
    ...
    try:
        ...
        try:
            ...
            if frame is not None:
                ...
                cmd = dbg.cmd_factory.make_error_message(
                    seq,
                    "internal_get_completions: Frame not found: %s from thread: %s"
                    % (frame_id, thread_id),
                )
                dbg.writer.add_command(cmd)
...
def internal_get_description(dbg, seq, thread_id, frame_id, expression):
    ...
    try:
        frame = dbg.find_frame(thread_id, frame_id)
        description = pydevd_console.get_description(
            frame, thread_id, frame_id, expression
        )
        ...
        exc = get_exception_traceback_str()
        cmd = dbg.cmd_factory.make_error_message(
            seq, "Error in fetching description" + exc
        )
        dbg.writer.add_command(cmd)


def build_exception_info_response(
    dbg,
    thread_id,
    thread,
    request_seq,
    set_additional_thread_info,
    iter_visible_frames_info,
    max_frames,
):
    """
                ...
                while frames_list is not None and len(frames_list):
                    ...
                    for (
                        ...
                        if not getattr(frame, "IS_PLUGIN_FRAME", False):
                            if dbg.is_files_filter_enabled and dbg.apply_files_filter(
                    ...
                    if IS_PY311_OR_GREATER:
                        ...
                        for (
                            ...
                            line_text,
                            line_col_info,
                        ) in frames[-max_frames:]:
                            if line_col_info is not None:
                                # End line might mean that we have a multiline statement.
                                if (
                                    line_col_info.end_lineno is not None
                                    and lineno < line_col_info.end_lineno
                                ):
                                    line_text = "\n".join(
                                        linecache.getlines(filename_in_utf8)[
                                            lineno : line_col_info.end_lineno + 1
                                        ]
                                    )
                        ...
                        stack_str = "".join(
                            traceback.format_list(
                                (x[:-1] for x in frames[-max_frames:])
                            )
                        )
    ...
    if current_paused_frame_name:
        name += (
            "       (note: full exception trace is shown but execution is paused at: %s)"
            % (current_paused_frame_name,)
        )
...
def internal_get_exception_details_json(
    ...
    thread,
    max_frames,
    set_additional_thread_info=None,
    iter_visible_frames_info=None,
):
    ...
    try:
        ...
        response = pydevd_base_schema.build_response(
            request, kwargs={"success": False, "message": exc, "body": {}}
        )
    ...
    def do_it(self, dbg):
        try:
            ...
            for filename, line, methodname, methodobj in self.stacktrace:
                ...
                callstack += (
                    '<frame thread_id = "%s" file="%s" line="%s" name="%s" obj="%s" />'
                    % (
                        self.thread_id,
                        makeValid(filename),
                        line,
            ...
            cmd = dbg.cmd_factory.make_send_breakpoint_exception_message(
                self.sequence, self.exc_type + "\t" + callstack
            )
            dbg.writer.add_command(cmd)
        except:
            exc = get_exception_traceback_str()
            sys.stderr.write("%s\n" % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error Sending Exception: " + exc
            )
            dbg.writer.add_command(cmd)
...
class InternalSendCurrExceptionTrace(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            cmd = dbg.cmd_factory.make_send_curr_exception_trace_message(
                dbg, self.sequence, self.thread_id, self.curr_frame_id, *self.arg
            )
            del self.arg
            dbg.writer.add_command(cmd)
        except:
            ...
            sys.stderr.write("%s\n" % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error Sending Current Exception Trace: " + exc
            )
            dbg.writer.add_command(cmd)
...
class InternalSendCurrExceptionTraceProceeded(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            cmd = dbg.cmd_factory.make_send_curr_exception_trace_proceeded_message(
                self.sequence, self.thread_id
            )
            dbg.writer.add_command(cmd)
        except:
            exc = get_exception_traceback_str()
            sys.stderr.write("%s\n" % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error Sending Current Exception Trace Proceeded: " + exc
            )
            dbg.writer.add_command(cmd)
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            if frame is not None:
                ...
                cmd = dbg.cmd_factory.make_send_console_message(
                    self.sequence, console_message.to_xml()
                )
            else:
                ...
                console_message.add_console_message(
                    ...
                    "Select the valid frame in the debug view (thread: %s, frame: %s invalid)"
                    % (self.thread_id, self.frame_id),
                )
                cmd = dbg.cmd_factory.make_error_message(
                    self.sequence, console_message.to_xml()
                )
...
class InternalRunCustomOperation(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            cmd = dbg.cmd_factory.make_custom_operation_message(
                self.sequence, resEncoded
            )
            dbg.writer.add_command(cmd)
        except:
            exc = get_exception_traceback_str()
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error in running custom operation" + exc
            )
            dbg.writer.add_command(cmd)
...
class InternalConsoleGetCompletions(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            cmd = dbg.cmd_factory.make_send_console_message(
                self.sequence, completions_xml
            )
            dbg.writer.add_command(cmd)
        except:
            exc = get_exception_traceback_str()
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error in fetching completions" + exc
            )
            dbg.writer.add_command(cmd)
        ...
        if is_current_thread_main_thread():
            for module in list(py_db.mpl_modules_for_patching):
                import_hook_manager.add_module_name(
                    module, py_db.mpl_modules_for_patching.pop(module)
                )
    ...
    def do_it(self, py_db):
        if not py_db.mpl_hooks_in_debug_console and not py_db.gui_in_use:
            ...
            try:
                ...
                pydev_log.debug(
                    "Matplotlib support in debug console failed", traceback.format_exc()
                )
        ...
        try:
            try:
                ...
                result = pydevconsole.console_exec(
                    self.thread_id, self.frame_id, self.expression, py_db
                )
                xml = "<xml>"
                xml += pydevd_xml.var_to_xml(result, "")
                xml += "</xml>"
                cmd = py_db.cmd_factory.make_evaluate_expression_message(
                    self.sequence, xml
                )
                py_db.writer.add_command(cmd)
            except:
                ...
                cmd = py_db.cmd_factory.make_error_message(
                    self.sequence, "Error evaluating console expression " + exc
                )
                py_db.writer.add_command(cmd)
        finally:
            enable_trace_thread_modules()
...
class InternalLoadFullValue(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        ...
        try:
            ...
            for variable in self.vars:
                ...
                if len(variable) > 0:
                    if "\t" in variable:  # there are attributes beyond scope
                        ...
                        name = scope
                    var_obj = pydevd_vars.getVariable(
                        dbg, self.thread_id, self.frame_id, scope, attrs
                    )
            ...
            sys.stderr.write("%s\n" % (exc,))
            cmd = dbg.cmd_factory.make_error_message(
                self.sequence, "Error evaluating variable %s " % exc
            )
            dbg.writer.add_command(cmd)
...
class AbstractGetValueAsyncThread(PyDBDaemonThread):
    ...
    def _on_run(self):
        ...
        for var_obj, name in self.var_objs:
            ...
            if (
                current_time - start > ASYNC_EVAL_TIMEOUT_SEC
...
class GetValueAsyncThreadDebug(AbstractGetValueAsyncThread):
    ...
    def send_result(self, xml):
        if self.frame_accessor is not None:
            cmd = self.frame_accessor.cmd_factory.make_load_full_value_message(
                self.seq, xml.getvalue()
            )
