...
def is_unhandled_exception(container_obj, py_db, frame, last_raise_line, raise_lines):
    ...
    if frame.f_lineno in raise_lines:
        ...
        if try_except_infos is None:
            container_obj.try_except_infos = try_except_infos = (
                py_db.collect_try_except_info(frame.f_code)
            )
...
class PyDBFrame:
    ...
    def trace_exception(self, frame, event, arg):
        ...
        if event == "exception":
            ...
            if should_stop:
                if handle_exception(
                    self._args[0], self._args[3], frame, arg, EXCEPTION_TYPE_HANDLED
                ):
            ...
            if exc_info and arg is None:
                ...
                if container_obj is None:
                    container_obj = frame_skips_cache[custom_key] = (
                        _TryExceptContainerObj()
                    )
                if is_unhandled_exception(
                    container_obj, self._args[0], frame, exc_info[1], exc_info[2]
                ) and self.handle_user_exception(frame):
                    return self.trace_dispatch
            ...
            if target_frame is not None and current_frame is not None:
                if target_frame.f_code.co_filename == current_frame.f_code.co_filename:
                    ...
                    if f is not None and f.f_code.co_name == PYDEVD_IPYTHON_CONTEXT[1]:
                        ...
                        if (
                            f is not None
                            and f.f_code.co_name == PYDEVD_IPYTHON_CONTEXT[2]
                        ):
                            return True
    ...
    def trace_dispatch(self, frame, event, arg):
        ...
        try:
            ...
            line = (
                frame.f_lineno or 0
            )  # Workaround or case where frame.f_lineno is None
            ...
            if (
                frame.f_code.co_flags & 0xA0
            ):  # 0xa0 ==  CO_GENERATOR = 0x20 | CO_COROUTINE = 0x80
                ...
                if event == "line":
                    ...
                    ):  # Check line to deal with async/await.
                        function_breakpoint_on_call_event = (
                            py_db.function_breakpoint_name_to_breakpoint.get(
            ...
            if not is_exception_event:
                breakpoints_for_file = py_db.breakpoints.get(
                    abs_path_canonical_path_and_base[1]
                )
                ...
                if info.pydev_state == 1:  # STATE_RUN = 1
                    ...
                    if step_cmd == -1:
                        ...
                        and (
                            ...
                            or stop_frame is not frame.f_back.f_back
                        )
                    ):
                        can_skip = True
                        ...
                        if py_db.apply_files_filter(
                            frame, frame.f_code.co_filename, True
                        ) and (
                            frame.f_back is None
                            or py_db.apply_files_filter(
                    ...
                    if can_skip:
                        ...
                        if (
                            ...
                            and info.pydev_step_cmd
                            in (CMD_STEP_OVER, CMD_STEP_OVER_MY_CODE)
            ...
            try:
                ...
                if function_breakpoint_on_call_event:
                    ...
                    and line in breakpoints_for_file
                ):
                    ...
                    result = plugin_manager.get_breakpoint(
                        py_db, frame, event, self._args[2]
                    )
                    if result:
                ...
                if breakpoint:
                    ...
                    if is_call and (
                        frame.f_code.co_name in ("<lambda>", "<module>")
                        or (line == 1 and frame.f_code.co_name.startswith("<cell"))
                    ):
                    ...
                    if (stop or stop_on_plugin_breakpoint) and breakpoint.is_logpoint:
                        ...
                        if (
                            info.pydev_message is not None
                ...
                if py_db.show_return_values:
                    if is_return and (
                        ...
                        or (
                            info.pydev_step_cmd
                            in (CMD_STEP_RETURN, CMD_STEP_RETURN_MY_CODE)
                            and (self._is_same_frame(stop_frame, frame))
                        )
                        or (
                            info.pydev_step_cmd
                            in (CMD_STEP_INTO, CMD_STEP_INTO_COROUTINE)
                        )
                        or (
                            info.pydev_step_cmd == CMD_STEP_INTO_MY_CODE
                            and frame.f_back is not None
                            and not py_db.apply_files_filter(
                                frame.f_back, frame.f_back.f_code.co_filename, True
                            )
                        )
                    ):
            ...
            try:
                ...
                if pydevd_dont_trace.should_trace_hook is not None:
                    if self.should_skip == -1:
                        ...
                        if not pydevd_dont_trace.should_trace_hook(
                            frame.f_code, abs_path_canonical_path_and_base[0]
                        ):
                    ...
                    if is_line:
                        if not info.pydev_use_scoped_step_frame:
                            if (
                                ...
                                or py_db.is_files_filter_enabled
                            ):
                            ...
                            if (
                                ...
                                if not not py_db.apply_files_filter(
                                    frame,
                                    frame.f_code.co_filename,
                                    force_check_project_scope,
                                ):
                            ...
                            if not filename.endswith(PYDEVD_IPYTHON_CONTEXT[0]):
                                ...
                                while f is not None:
                                    if f.f_code.co_name == PYDEVD_IPYTHON_CONTEXT[1]:
                                        ...
                                        if (
                                            f2 is not None
                                            and f2.f_code.co_name
                                            == PYDEVD_IPYTHON_CONTEXT[2]
                                        ):
                        ...
                        if py_db.get_file_type(frame.f_back) == py_db.PYDEV_FILE:
                            ...
                            if (
                                ...
                                if stop:
                                    ...
                                    if info.step_in_initial_location == (
                                        frame.f_back,
                    ...
                    if self._is_same_frame(stop_frame, frame) and is_return:
                        ...
                        if info.pydev_smart_child_offset != -1:
                            ...
                            pydev_smart_step_into_variants = (
                                info.pydev_smart_step_into_variants
                            )
                            if (
                                pydev_smart_parent_offset >= 0
                                and pydev_smart_step_into_variants
                            ):
                                ...
                                # global context is set with an empty name
                                if (
                                    curr_func_name in ("?", "<module>")
                                    or curr_func_name is None
                                ):
                                    curr_func_name = ""
                                if (
                                    curr_func_name == info.pydev_func_name
                                    and stop_frame.f_lineno == info.pydev_next_line
                                ):
                                    stop = True
                        ...
                        back is not None
                        and self._is_same_frame(stop_frame, back.f_back)
                        and is_line
                    ):
                        ...
                        stop = False
                        if (
                            pydev_smart_child_offset >= 0
                            and pydev_smart_child_offset >= 0
                        ):
                            pydev_smart_step_into_variants = (
                                info.pydev_smart_step_into_variants
                            )
                            ...
                            if (
                                ...
                                stop = children_variants and (
                                    get_smart_step_into_variant_from_frame_offset(
                                        back.f_lasti, children_variants
                                    )
                                    is get_smart_step_into_variant_from_frame_offset(
                                        pydev_smart_child_offset, children_variants
                                    )
                                )
                ...
                if plugin_stop:
                    plugin_manager.stop(
                        py_db, frame, event, self._args[3], stop_info, arg, step_cmd
                    )
                elif stop:
                    if is_line:
                        self.set_suspend(
                        ...
                        if back is not None:
                            ...
                            back_absolute_filename, _, base = (
                                get_abs_path_real_path_and_base_from_frame(back)
                            )
                            if (base, back.f_code.co_name) in (
                                DEBUG_START,
                                DEBUG_START_PY3K,
                            ):
                                back = None
                                ...
                                if not pydevd_dont_trace.should_trace_hook(
                                    back.f_code, back_absolute_filename
                                ):
    ...
    if info.pydev_state != 2:  # and breakpoint is not None:
        ...
        if trace is not None and hasattr(trace, "tb_next"):
                ...
                if exception == SystemExit and py_db.ignore_system_exit_code(value):
                    ...
                    for exc_break, is_user_uncaught in check_excs:
                        ...
                        if py_db.exclude_exception_by_filter(exc_break, trace):
                            ...
                            exc_break.condition is not None
                            and not py_db.handle_breakpoint_condition(
                                info, exc_break, frame
                            )
                        ):
                            should_stop = False
                            ...
                            should_stop = False
                            if not py_db.apply_files_filter(
                                frame, frame.f_code.co_filename, True
                            ) and (
                                frame.f_back is None
                                or py_db.apply_files_filter(
                            ...
                            if (
                                ...
                                should_stop = (
                                    False  # I.e.: we stop only when it was just raised
                                )
            ...
            if should_stop:
                ...
                if (
                    exception_breakpoint is not None
                    and exception_breakpoint.expression is not None
                ):
                    py_db.handle_breakpoint_expression(
    ...
    try:
        ...
        if py_db.ignore_exceptions_thrown_in_lines_with_ignore_exception:
            for check_trace_obj in (initial_trace_obj, trace_obj):
                abs_real_path_and_base = get_abs_path_real_path_and_base_from_frame(
                    check_trace_obj.tb_frame
                )
                ...
                if last_stat != curr_stat:
                    ...
                    try:
                        ...
                        pydev_log.exception(
                            "Error in linecache.checkcache(%r)", absolute_filename
                        )

                from_user_input = (
                    py_db.filename_to_lines_where_exceptions_are_ignored.get(
                        canonical_normalized_filename
                    )
                )
                if from_user_input:
                ...
                if (
                    ...
                    try:
                        ...
                        pydev_log.exception(
                            "Error in linecache.getline(%r, %s, f_globals)",
                            absolute_filename,
                            exc_lineno,
                        )
                        line = ""
        ...
        try:
            ...
            try:
                ...
                py_db.do_wait_suspend(
                    thread, frame, "exception", arg, exception_type=exception_type
                )
            finally:
