...
import sys
import traceback
from _pydevd_bundle.pydevconsole_code import (
    InteractiveConsole,
    _EvalAwaitInNewEventLoop,
)
            ...
            if is_async:
                ...
                try:
                    ...
                    update_globals_and_locals(
                        updated_globals, initial_globals, self.frame
                    )
        except SystemExit:
            raise
        except:
...
def get_interactive_console(thread_id, frame_id, frame, console_message):
    ...
    if (
        ...
        and InteractiveConsoleCache.frame_id == frame_id
    ):
    ...
    if console_stacktrace:
        ...
        context_message = 'File "%s", line %s, in %s' % (
            current_context[0],
            current_context[1],
            current_context[2],
        )
        console_message.add_console_message(
            CONSOLE_OUTPUT, "[Current context]: %s" % (context_message,)
        )
    return InteractiveConsoleCache.interactive_console_instance
