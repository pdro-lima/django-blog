...
def fix_top_level_trace_and_get_trace_func(py_db, frame):
    ...
    while f_unhandled is not None:
        ...
        if name == "threading":
            ...
            elif f_unhandled.f_code.co_name in (
                "__bootstrap_inner",
                "_bootstrap_inner",
            ):
    ...
    if f_unhandled is not None:
        if f_unhandled.f_back is None and not force_only_unhandled_tracer:
            ...
            top_level_thread_tracer = TopLevelThreadTracerNoBackFrame(
                ThreadTracer(args), args
            )
            ...
            if top_level_thread_tracer is None:
                ...
                top_level_thread_tracer = TopLevelThreadTracerOnlyUnhandledExceptions(
                    args
                )
...
def trace_dispatch(py_db, frame, event, arg):
    thread_trace_func, apply_to_settrace = py_db.fix_top_level_trace_and_get_trace_func(
        py_db, frame
    )
