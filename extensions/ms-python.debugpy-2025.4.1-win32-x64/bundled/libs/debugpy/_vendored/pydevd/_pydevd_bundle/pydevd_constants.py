...
try:
    ...
    def get_frame():
        raise AssertionError(
            "sys._getframe not available (possible causes: enable -X:Frames on IronPython?)"
        )
...
if USE_CUSTOM_SYS_CURRENT_FRAMES:
    ...
    if IS_JYTHON:
        ...
        try:
            cachedThreadState = ThreadStateMapping.getDeclaredField(
                "globalThreadStates"
            )  # Dev version
        except NoSuchFieldException:
            cachedThreadState = ThreadStateMapping.getDeclaredField(
                "cachedThreadState"
            )  # Release Jython 2.7.0
...
if (
...
):  # Default gotten, let's see if it was somehow customize by the user.
    ...
    if _use_sys_monitoring_env_var:
        ...
        if _use_sys_monitoring_env_var in ENV_FALSE_LOWER_VALUES:
            ...
            raise RuntimeError(
                "Unrecognized value for PYDEVD_USE_SYS_MONITORING: %s"
                % (_use_sys_monitoring_env_var,)
            )
...
INTERACTIVE_MODE_AVAILABLE = (
    sys.platform in ("darwin", "win32") or os.getenv("DISPLAY") is not None
)
...
if USE_CYTHON_FLAG is not None:
    ...
    if (
        USE_CYTHON_FLAG not in ENV_TRUE_LOWER_VALUES
        and USE_CYTHON_FLAG not in ENV_FALSE_LOWER_VALUES
    ):
        raise RuntimeError(
...
#
PYDEVD_CONTAINER_INITIAL_EXPANDED_ITEMS = as_int_in_env(
    "PYDEVD_CONTAINER_INITIAL_EXPANDED_ITEMS", 100
)
PYDEVD_CONTAINER_BUCKET_SIZE = as_int_in_env("PYDEVD_CONTAINER_BUCKET_SIZE", 1000)
PYDEVD_CONTAINER_RANDOM_ACCESS_MAX_ITEMS = as_int_in_env(
    "PYDEVD_CONTAINER_RANDOM_ACCESS_MAX_ITEMS", 500
)
PYDEVD_CONTAINER_NUMPY_MAX_ITEMS = as_int_in_env(
    "PYDEVD_CONTAINER_NUMPY_MAX_ITEMS", 500
)

PYDEVD_IPYTHON_COMPATIBLE_DEBUGGING = is_true_in_env(
...
if _ipython_ctx:
    ...
    assert len(PYDEVD_IPYTHON_CONTEXT) == 3, "Invalid PYDEVD_IPYTHON_CONTEXT: %s" % (
        _ipython_ctx,
    )
...
SHOW_COMPILE_CYTHON_COMMAND_LINE = is_true_in_env(
    "PYDEVD_SHOW_COMPILE_CYTHON_COMMAND_LINE"
)
...
PYDEVD_WARN_SLOW_RESOLVE_TIMEOUT = as_float_in_env(
    "PYDEVD_WARN_SLOW_RESOLVE_TIMEOUT", 0.50
)
...
PYDEVD_APPLY_PATCHING_TO_HIDE_PYDEVD_THREADS = (
    os.getenv("PYDEVD_APPLY_PATCHING_TO_HIDE_PYDEVD_THREADS", "true").lower()
    in ENV_TRUE_LOWER_VALUES
)
