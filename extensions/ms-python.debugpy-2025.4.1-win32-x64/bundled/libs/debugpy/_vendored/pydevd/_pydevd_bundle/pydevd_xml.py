...
from _pydevd_bundle.pydevd_extension_api import (
    TypeResolveProvider,
...
def _create_default_type_map():
    ...
    default_type_map.append(
        (MoreItems, pydevd_resolver.forwardInternalResolverToObject)
    )
    default_type_map.append(
        (MoreItemsRange, pydevd_resolver.forwardInternalResolverToObject)
    )
    ...
    try:
        ...
        default_type_map.insert(
            0, (MultiValueDict, pydevd_resolver.multiValueDictResolver)
        )
        # we should put it before dict
    except:
...
class TypeResolveHandler(object):
    NO_PROVIDER = (
        []
    )  # Sentinel value (any mutable object to be used as a constant would be valid).
    ...
    def str_from_providers(
        self, o, type_object, type_name, context: Optional[str] = None
    ):
        ...
        for provider in self._str_providers:
            if provider.can_provide(type_object, type_name):
                ...
                try:
                    ...
                    pydev_log.exception(
                        "Error when getting str with custom provider: %s." % (provider,)
                    )
...
def should_evaluate_full_value(val):
    return not LOAD_VALUES_ASYNC or (
        is_builtin(type(val)) and not isinstance_checked(val, (list, tuple, dict))
    )
                ...
                if v.__class__ == frame_type:
                    ...
                    try:
                        ...
                        if cName.find(".") != -1:
                            ...
                            cName.find("'") != -1
                        ):  # does not have '.' (could be something like <type 'int'>)
...
def var_to_xml(
    ...
    type_name, type_qualifier, is_exception_on_eval, resolver, value = (
        get_variable_details(val, evaluate_full_value)
    )
    ...
    if scope:
        ...
        return "".join(
            (xml, xml_qualifier, xml_value, xml_container, additional_in_xml, " />\n")
        )
