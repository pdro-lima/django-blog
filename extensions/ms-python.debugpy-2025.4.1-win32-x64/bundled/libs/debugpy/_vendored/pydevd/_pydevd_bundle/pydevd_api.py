        ...
        def iterate():
            ...
            for const in code.co_consts:
                if (
                    ...
                    and const.co_filename == code.co_filename
                ):
...
class PyDevdAPI(object):
    class VariablePresentation(object):
        def __init__(
            self, special="group", function="group", class_="group", protected="inline"
        ):
            self._presentation = {
    ...
    def request_exception_info_json(
    ...
    def request_smart_step_into_by_func_name(
        self, py_db, seq, thread_id, line, func_name
    ):
        # Same thing as set next, just with a different cmd id.
        self.request_set_next(
            py_db, seq, thread_id, CMD_SMART_STEP_INTO, None, line, func_name
        )

    def request_set_next(
        ...
        if t:
            if original_filename is not None:
                ...
                assert (
                    translated_filename.__class__ == str
                ...
                _source_mapped_filename, new_line, multi_mapping_applied = (
                    py_db.source_mapping.map_to_server(translated_filename, line)
                )
            ...
            int_cmd = InternalSetNextStatementThread(
                thread_id, set_next_cmd_id, line, func_name, seq=seq
            )
    ...
    def request_get_array(
        ...
        int_cmd = InternalGetArray(
            seq, roffset, coffset, rows, cols, fmt, thread_id, frame_id, scope, attrs
        )
        py_db.post_internal_command(int_cmd, thread_id)
    ...
    def to_str(self, s):
        ...
        if s.__class__ != str:
            raise AssertionError(
                "Expected to have str on Python 3. Found: %s (%s)" % (s, s.__class__)
            )
        return s
    ...
    def filename_to_str(self, filename):
        ...
        if filename.__class__ != str:
            raise AssertionError(
                "Expected to have str on Python 3. Found: %s (%s)"
                % (filename, filename.__class__)
            )
        return filename
    ...
    class _AddBreakpointResult(object):
        ...
        def __init__(
            self, breakpoint_id, translated_filename, translated_line, original_line
        ):
    ...
    def add_breakpoint(
        ...
        original_filename_normalized = pydevd_file_utils.normcase_from_client(
            original_filename
        )

        pydev_log.debug(
            "Request for breakpoint in: %s line: %s", original_filename, line
        )
        original_line = line
        ...
        pydev_log.debug(
            "Breakpoint (after path translation) in: %s line: %s",
            translated_filename,
            line,
        )
        func_name = self.to_str(func_name)
        ...
        # Apply source mapping (i.e.: ipython).
        source_mapped_filename, new_line, multi_mapping_applied = (
            py_db.source_mapping.map_to_server(translated_filename, line)
        )
        ...
        if multi_mapping_applied:
            ...
            result = self._AddBreakpointResult(
                breakpoint_id, original_filename, line, original_line
            )
        ...
        py_db.api_received_breakpoints[
            (original_filename_normalized, breakpoint_id)
        ] = (
    ...
    def reapply_breakpoints(self, py_db):
        ...
        values = list(
            py_db.api_received_breakpoints.values()
        )  # Create a copy with items to reapply.
        self.remove_all_breakpoints(py_db, "*")
        for val in values:
    ...
    def remove_all_breakpoints(self, py_db, received_filename):
        ...
        if received_filename == "*":
            ...
            received_filename_normalized = pydevd_file_utils.normcase_from_client(
                received_filename
            )
            items = list(
                py_db.api_received_breakpoints.items()
    ...
    def remove_breakpoint(
        ...
        for key, val in list(py_db.api_received_breakpoints.items()):
            ...
            if (
                received_filename_normalized == original_filename_normalized
                and existing_breakpoint_id == breakpoint_id
            ):
                del py_db.api_received_breakpoints[key]
                break
        else:
    ...
    def set_function_breakpoints(self, py_db, function_breakpoints):
        ...
        for function_breakpoint in function_breakpoints:
            function_breakpoint_name_to_breakpoint[function_breakpoint.func_name] = (
                function_breakpoint
            )

        py_db.function_breakpoint_name_to_breakpoint = (
            function_breakpoint_name_to_breakpoint
        )
        py_db.on_breakpoints_changed()
    ...
    def request_load_source(self, py_db, seq, filename):
        ...
        try:
            ...
            cmd = py_db.cmd_factory.make_error_message(
                seq, get_exception_traceback_str()
            )
    ...
    def get_decompiled_source_from_frame_id(self, py_db, frame_id):
        ...
        lines = (
            linecache.getline(frame.f_code.co_filename, i) for i in itertools.count(1)
        )
    ...
    def add_plugins_exception_breakpoint(self, py_db, breakpoint_type, exception):
        ...
        if plugin is not None:
            supported_type = plugin.add_breakpoint(
                "add_exception_breakpoint", py_db, breakpoint_type, exception
            )
    ...
    def remove_plugins_exception_breakpoint(self, py_db, exception_type, exception):
        ...
        if supported_type:
            ...
            pydev_log.info(
                "No exception of type: %s was previously registered.", exception_type
            )
    ...
    def set_dont_trace_start_end_patterns(self, py_db, start_patterns, end_patterns):
        ...
        dont_trace_start_end_patterns_previously_set = (
            ...
            == "custom_dont_trace_external_files"
        )

        if (
            not dont_trace_start_end_patterns_previously_set
            and not start_patterns
            and not end_patterns
        ):
        ...
        def custom_dont_trace_external_files(abs_path):
            ...
            return normalized_abs_path.startswith(
                start_patterns
            ) or normalized_abs_path.endswith(end_patterns)
    ...
    def stop_on_entry(self):
        ...
        if main_thread is None:
            pydev_log.critical(
                "Could not find main thread while setting Stop on Entry."
            )
        else:
            ...
            if py_db.terminate_child_processes:
                ...
                if IS_WINDOWS:
                    self._terminate_child_processes_windows(
                        py_db.dont_terminate_child_pids
                    )
                else:
    ...
    def setup_auto_reload_watcher(
        ...
        py_db.setup_auto_reload_watcher(
            enable_auto_reload,
            watch_dirs,
            poll_target_time,
            exclude_patterns,
...
def _list_ppid_and_pid():
    ...
    try:
        ...
        if not kernel32.Process32First(
            ctypes.c_void_p(snapshot), ctypes.byref(process_entry)
        ):
            pydev_log.critical(
            ...
            while True:
                ppid_and_pids.append(
                    (process_entry.th32ParentProcessID, process_entry.th32ProcessID)
                )
