...
class _StackInterpreter(object):
    ...
    def on_LOAD_ATTR(self, instr):
        ...
        self._stack.append(instr)
        self.load_attrs[_TargetIdHashable(instr)] = Target(
            self._getname(instr), instr.lineno, instr.offset
        )
    ...
    def _handle_call_from_instr(self, func_name_instr, func_call_instr):
        ...
        if not call_name:
            ...
            code_obj = self.func_name_id_to_code_object[
                _TargetIdHashable(func_name_instr)
            ]
            if code_obj is not None and GO_INTO_INNER_CODES:
                ...
                if children_targets:
                    ...
                    target = Target(
                        call_name,
                        func_name_instr.lineno,
                        func_call_instr.offset,
                        children_targets,
                    )
    ...
    def on_COMPARE_OP(self, instr):
        ...
        if cmp_op not in ("exception match", "BAD"):
            self.function_calls.append(
                Target(self._getname(instr), instr.lineno, instr.offset)
            )
    ...
    def on_BINARY_SUBSCR(self, instr):
        ...
        try:
            ...
            return
        self.function_calls.append(
            Target(_BINARY_OP_MAP[instr.name], instr.lineno, instr.offset)
        )
        self._stack.append(instr)
    ...
    def on_LOAD_METHOD(self, instr):
        self.on_POP_TOP(
            instr
        )  # Remove the previous as we're loading something from it.
        self._stack.append(instr)
    ...
    def on_MAKE_FUNCTION(self, instr):
        ...
        if call_name in ("<genexpr>", "<listcomp>", "<setcomp>", "<dictcomp>"):
            if isinstance(code_obj_instr.arg, CodeType):
                self.func_name_id_to_code_object[_TargetIdHashable(qualname)] = (
                    code_obj_instr.arg
                )
    ...
    def __repr__(self):
        ...
        for s in self.__slots__:
            ...
            if s == "children_variants":
                ret.append(
                    "children_variants: %s"
                    % (len(self.children_variants) if self.children_variants else 0)
                )
                continue
...
def _convert_target_to_variant(
    ...
    if children_targets:
        children_variants = [
            _convert_target_to_variant(
                child, start_line, end_line, call_order_cache, lasti, base
            )
...
def calculate_smart_step_into_variants(frame, start_line, end_line, base=0):
    ...
    if IS_PY311_OR_GREATER:
        ...
        return pydevd_bytecode_utils_py311.calculate_smart_step_into_variants(
            frame, start_line, end_line, base
        )
    ...
    for target in _get_smart_step_into_targets(code):
        variant = _convert_target_to_variant(
            target, start_line, end_line, call_order_cache, lasti, base
        )
        if variant is None:
            continue
