...
from _pydevd_bundle.pydevd_net_command import (
    NetCommand,
    NULL_NET_COMMAND,
    NULL_EXIT_COMMAND,
)
...
class NetCommandFactory(object):
    ...
    def make_custom_frame_created_message(self, frame_id, frame_description):
        ...
        return NetCommand(
            CMD_THREAD_CREATE,
            0,
            '<xml><thread name="%s" id="%s"/></xml>' % (frame_description, frame_id),
        )
    ...
    def make_get_thread_stack_message(
        self,
        py_db,
        seq,
        thread_id,
    ...
    def _iter_visible_frames_info(self, py_db, frames_list, flatten_chained=False):
        ...
        while True:
            for frame in frames_list:
                ...
                if is_chained:
                    method_name = "[Chained Exc: %s] %s" % (
                        frames_list.exc_desc,
                        method_name,
                    )
                ...
                if (
                    py_db.get_file_type(frame, abs_path_real_path_and_base)
                    == py_db.PYDEV_FILE
                ):
                    # Skip pydevd files.
                    frame = frame.f_back
                ...
                new_filename_in_utf8, applied_mapping = (
                    pydevd_file_utils.map_file_to_client(filename_in_utf8)
                )
    ...
    def make_thread_stack_str(self, py_db, frames_list):
        ...
        try:
            for (
                ...
                line_col_info,
            ) in self._iter_visible_frames_info(
                ...
                append(
                    'file="%s" line="%s">'
                    % (quote(make_valid_xml_value(filename_in_utf8), "/>_= \t"), lineno)
                )
                append("</frame>")
        except:
            pydev_log.exception()
    ...
    def make_thread_suspend_message(
        self,
        py_db,
        thread_id,
        frames_list,
    ...
    def make_thread_suspend_single_notification(
        self, py_db, thread_id, thread, stop_reason
    ):
        try:
            return NetCommand(
                CMD_THREAD_SUSPEND_SINGLE_NOTIFICATION,
                0,
                json.dumps({"thread_id": thread_id, "stop_reason": stop_reason}),
            )
        except:
            return self.make_error_message(0, get_exception_traceback_str())
    ...
    def make_reloaded_code_message(self, seq, reloaded_ok):
        try:
            return NetCommand(
                CMD_RELOAD_CODE,
                seq,
                '<xml><reloaded ok="%s"></reloaded></xml>' % reloaded_ok,
            )
        except Exception:
            return self.make_error_message(seq, get_exception_traceback_str())
    ...
    def _make_send_curr_exception_trace_str(
        self, py_db, thread_id, exc_type, exc_desc, trace_obj
    ):
        frames_list = pydevd_frame_utils.create_frames_list_from_traceback(
            trace_obj, None, exc_type, exc_desc
        )

        exc_type = (
            pydevd_xml.make_valid_xml_value(str(exc_type)).replace("\t", "  ")
            or "exception: type unknown"
        )
        exc_desc = (
            pydevd_xml.make_valid_xml_value(str(exc_desc)).replace("\t", "  ")
            or "exception: no description"
        )
    ...
    def make_send_curr_exception_trace_message(
        self, py_db, seq, thread_id, curr_frame_id, exc_type, exc_desc, trace_obj
    ):
        try:
            exc_type, exc_desc, thread_suspend_str, _thread_stack_str = (
                self._make_send_curr_exception_trace_str(
            ...
            if topmost_frame is not None:
                try:
                    ...
                    while frame is not None:
                        if (
                            frame.f_code.co_name == "do_wait_suspend"
                            and frame.f_code.co_filename.endswith("pydevd.py")
                        ):
    ...
    def make_send_curr_exception_trace_proceeded_message(self, seq, thread_id):
        try:
            return NetCommand(
                CMD_SEND_CURR_EXCEPTION_TRACE_PROCEEDED, 0, str(thread_id)
            )
        except:
            return self.make_error_message(0, get_exception_traceback_str())
...
""" % (
            expression,
            pydevd_constants.PYDEVD_WARN_EVALUATION_TIMEOUT,
        )
