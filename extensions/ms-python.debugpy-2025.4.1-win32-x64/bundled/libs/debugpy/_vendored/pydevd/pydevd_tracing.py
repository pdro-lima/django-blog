...
def _get_stack_str(frame):
    ...
    if TracingFunctionHolder._traceback_limit:
        ...
        traceback.print_stack(
            f=frame, limit=TracingFunctionHolder._traceback_limit, file=s
        )
        msg = msg + s.getvalue()
...
def _internal_set_trace(tracing_func):
    if PYDEVD_USE_SYS_MONITORING:
        raise RuntimeError(
            "pydevd: Using sys.monitoring, sys.settrace should not be called."
        )
        ...
        if frame is not None and frame.f_back is not None:
            ...
            if filename.endswith("threadpool") and "gevent" in filename:
                if tracing_func is None:
                    pydev_log.debug(
                        "Disabled internal sys.settrace from gevent threadpool."
                    )
                    return
    ...
    if tracing_func is not None:
        if (
            set_trace_to_threads(
                tracing_func,
                thread_idents=[thread.get_ident()],
                create_dummy_thread=False,
            )
            == 0
        ):
...
def get_python_helper_lib_filename():
    ...
    libdir = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "pydevd_attach_to_process"
    )
    ...
    if IS_WINDOWS:
        ...
        pydev_log.info(
            "Unable to set trace to all threads in platform: %s", sys.platform
        )
        return None
    ...
    for thread_ident in thread_idents:
        ...
        if lib is None:  # This is the case if it's not CPython.
            ...
            try:
                ...
                if result != 0:
                    pydev_log.info(
                        "Unable to set tracing for existing thread. Result: %s", result
                    )
                    ret = result
