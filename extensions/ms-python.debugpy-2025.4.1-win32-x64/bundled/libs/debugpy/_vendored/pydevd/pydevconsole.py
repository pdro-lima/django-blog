...
from _pydev_bundle.pydev_console_utils import (
...
def init_set_return_control_back(interpreter):
    ...
    def return_control():
        ...
        if _ProcessExecQueueHelper._debug_hook:
            ...
            _ProcessExecQueueHelper._return_control_osc = (
                not _ProcessExecQueueHelper._return_control_osc
            )
...
def init_mpl_in_console(interpreter):
    ...
    for mod in list(interpreter.mpl_modules_for_patching):
        import_hook_manager.add_module_name(
            mod, interpreter.mpl_modules_for_patching.pop(mod)
        )
...
if sys.platform != "win32":
    ...
    def pid_exists(pid):
        ...
        process = kernel32.OpenProcess(
            PROCESS_QUERY_INFORMATION | PROCESS_QUERY_LIMITED_INFORMATION, 0, pid
        )
        if not process:
        ...
        try:
            ...
            if not exit_code_suceeded:
                ...
                bool(exit_code.contents.value)
                and int(exit_code.contents.value) != STILL_ACTIVE
            ):
                return False
        finally:
...
def start_console_server(host, port, interpreter):
    try:
        ...
        from _pydev_bundle.pydev_imports import (
            SimpleXMLRPCServer as XMLRPCServer,
        )  # @Reimport
...
def get_interpreter():
    try:
        ...
        interpreterInterface = InterpreterInterface(
            None, None, threading.current_thread()
        )
...
def console_exec(thread_id, frame_id, expression, dbg):
    ...
    if IPYTHON:
        need_more = exec_code(
            CodeFragment(expression), updated_globals, frame.f_locals, dbg
        )
        if not need_more:
...
if __name__ == "__main__":
    ...
    pydevconsole.start_server(
        pydev_localhost.get_localhost(), int(port), int(client_port)
    )
