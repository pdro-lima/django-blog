...
def get_target_filename(is_target_process_64=None, prefix=None, extension=None):
    ...
    if is_target_process_64 is None:
        if IS_WINDOWS:
            ...
            raise AssertionError(
                "On windows it's expected that the target bitness is specified."
            )
    ...
    if not arch:
        ...
        if not arch:
            print(
                "platform.machine() did not return valid value."
    ...
    if arch.lower() not in ("amd64", "x86", "x86_64", "i386", "x86"):
        ...
        filename = None
        if (
            expected_name in found
        ):  # Heuristic: user compiled with "attach_<arch>.<extension>"
            filename = os.path.join(libdir, expected_name)

        elif (
            IS_LINUX and expected_name_linux in found
        ):  # Heuristic: user compiled with "attach_linux_<arch>.<extension>"
            filename = os.path.join(libdir, expected_name_linux)

        elif (
            len(found) == 1
        ):  # Heuristic: user removed all libraries and just left his own lib.
            ...
            filtered = [
                name
                for name in found
                if not name.endswith((suffix_64 + extension, suffix_32 + extension))
            ]
...
def run_python_code_windows(
    ...
    with _acquire_mutex("_pydevd_pid_attach_mutex_%s" % (pid,), 10):
        print(
            "--- Connecting to %s bits target (current process is: %s) ---"
            % (bits, 64 if is_python_64bit() else 32)
        )
        sys.stdout.flush()

        with _win_write_to_shared_named_memory(python_code, pid):
            target_executable = get_target_filename(
                is_target_process_64, "inject_dll_", ".exe"
            )
            if not target_executable:
                raise RuntimeError(
                    "Could not find expected .exe file to inject dll in attach to process."
                )

            target_dll = get_target_filename(is_target_process_64)
            if not target_dll:
                raise RuntimeError(
                    "Could not find expected .dll file in attach to process."
                )

            print(
                "\n--- Injecting attach dll: %s into pid: %s ---"
                % (os.path.basename(target_dll), pid)
            )
            sys.stdout.flush()
            ...
            # run the code.
            target_dll_run_on_dllmain = get_target_filename(
                is_target_process_64, "run_code_on_dllmain_", ".dll"
            )
            ...
            with _create_win_event("_pydevd_pid_event_%s" % (pid,)) as event:
                print(
                    "\n--- Injecting run code dll: %s into pid: %s ---"
                    % (os.path.basename(target_dll_run_on_dllmain), pid)
                )
                sys.stdout.flush()
...
def _win_write_to_shared_named_memory(python_code, pid):
    ...
    assert (
        len(python_code) < BUFSIZE - 1
    ), "Python code must have at most %s bytes (found: %s)" % (
        BUFSIZE - 1,
        len(python_code),
    )
    ...
    filemap = CreateFileMapping(
        ...
        0,
        BUFSIZE,
        "__pydevd_pid_code_to_run__%s" % (pid,),
    )
...
def run_python_code_linux(
    ...
    gdb_load_shared_libraries = os.environ.get(
        "PYDEVD_GDB_SCAN_SHARED_LIBRARIES", ""
    ).strip()
    if gdb_load_shared_libraries:
        print(
            "PYDEVD_GDB_SCAN_SHARED_LIBRARIES set: %s." % (gdb_load_shared_libraries,)
        )
        cmd.extend(
            ["--init-eval-command='set auto-solib-add off'"]
        )  # Don't scan all libraries.

        for lib in gdb_load_shared_libraries.split(","):
            lib = lib.strip()
            cmd.extend(
                ["--eval-command='sharedlibrary %s'" % (lib,)]
...
def run_python_code_mac(
    ...
    cmd.extend(
        [
            ...
            '-o \'load_lib_and_attach "%s" %s "%s" %s\''
            % (target_dll, is_debug, python_code, show_debug_info),
        ]
    )
