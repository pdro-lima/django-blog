    ...
    for version in ("38", "39"):
        ...
        for line in template_lines:
            if version == "38":
                ...
                line = line.replace(
                    "CALL_EvalFrameDefault", "CALL_EvalFrameDefault_38(frame_obj, exc)"
                )
            else:  # 3.9
                line = line.replace(
                    "get_bytecode_while_frame_eval(PyFrameObject * frame_obj, int exc)",
                ...
                line = line.replace(
                    "CALL_EvalFrameDefault",
                    "CALL_EvalFrameDefault_39(tstate, frame_obj, exc)",
                )
...
def build_extension(
    ...
    pyx_file = os.path.join(
        os.path.dirname(__file__), dir_name, "%s.pyx" % (extension_name,)
    )

    if template:
        pyx_template_file = os.path.join(
            os.path.dirname(__file__), dir_name, "%s.template.pyx" % (extension_name,)
        )
    ...
    if target_pydevd_name != extension_name:
        ...
        new_pyx_file = os.path.join(
            os.path.dirname(__file__), dir_name, "%s.pyx" % (target_pydevd_name,)
        )
        new_c_file = os.path.join(
            os.path.dirname(__file__), dir_name, "%s.c" % (target_pydevd_name,)
        )
        shutil.copy(pyx_file, new_pyx_file)
        pyx_file = new_pyx_file
        if has_pxd:
            pxd_file = os.path.join(
                os.path.dirname(__file__), dir_name, "%s.pxd" % (extension_name,)
            )
            new_pxd_file = os.path.join(
                os.path.dirname(__file__), dir_name, "%s.pxd" % (target_pydevd_name,)
            )
...
""",
                ...
                c_file_contents = c_file_contents.replace(
                    r"_pydevd_frame_eval\\release_mem.h",
                    "_pydevd_frame_eval/release_mem.h",
                )
                c_file_contents = c_file_contents.replace(
                    r"_pydevd_frame_eval\\pydevd_frame_evaluator.pyx",
                    "_pydevd_frame_eval/pydevd_frame_evaluator.pyx",
                )
                c_file_contents = c_file_contents.replace(
                    r"_pydevd_bundle\\pydevd_cython.pxd",
                    "_pydevd_bundle/pydevd_cython.pxd",
                )
                c_file_contents = c_file_contents.replace(
                    r"_pydevd_bundle\\pydevd_cython.pyx",
...
if extension_folder:
    ...
    for folder in [
        ...
        if file != "build" and os.path.isdir(os.path.join(extension_folder, file))
    ]:
