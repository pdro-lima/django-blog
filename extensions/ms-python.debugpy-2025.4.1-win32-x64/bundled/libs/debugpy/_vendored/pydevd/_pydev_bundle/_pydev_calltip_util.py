...
def get_description(obj):
    ...
    doc_string = (
        get_docstring(ob_call) if is_bound_method(ob_call) else get_docstring(obj)
    )
...
def create_method_stub(fn_name, fn_class, argspec, doc_string):
    if fn_name and argspec:
        ...
        fn_stub = create_function_stub(
            fn_name, argspec, doc_string, indent=1 if fn_class else 0
        )
        if fn_class:
        ...
        if fn_name:
            ...
            if restored_signature:
                return create_method_stub(
                    fn_name, fn_class, restored_signature, doc_string
                )
        return (
            create_function_stub("unknown", "(*args, **kwargs)", doc_string)
            + "\nunknown"
        )
