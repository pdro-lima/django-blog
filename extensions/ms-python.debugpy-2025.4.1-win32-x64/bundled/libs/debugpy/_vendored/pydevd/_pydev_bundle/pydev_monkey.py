...
def _separate_future_imports(code):
    ...
    try:
        ...
        if hasattr(node, "end_lineno") and hasattr(node, "end_col_offset"):
            ...
            offset = _get_offset_from_line_col(
                code, line - 1, col
            )  # ast lines are 1-based, make it 0-based.
    ...
    if sys.platform == "win32":
        ...
        for x in args:
            ...
            if x != two_double_quotes:
                if (
                    len(x) > 1
                    and x.startswith(double_quote)
                    and x.endswith(double_quote)
                ):
                    x = x[1:-1]
...
def patch_args(args, is_exec=False):
    ...
    try:
        ...
        try:
            ...
            pydev_log.info(
                "Unable to monkey-patch subprocess arguments because a type found in the args is invalid: %s",
                e,
            )
        ...
        ignore_next = (
            True  # start ignoring the first (the first entry is the python executable)
        )
                    ...
                    if c in params_with_combinable_arg:
                        ...
                        if c == "m":
                            ...
                            before_module_flag = arg_as_str[
                                :j
                            ]  # before_module_flag would then be "-q"
                            ...
                            # python -c "import sys"
                            # python "-qcimport sys"
                            code_flag = arg_as_str[
                                : j + 1
                            ]  # code_flag would then be "-qc"
        ...
        if code_i != -1:
            ...
            if port is not None:
                ...
                new_args.append(
                    _get_python_c_args(
                        host, port, code, unquoted_args, SetupHolder.setup
                    )
                )
        ...
        new_args.extend(
            setup_to_argv(
                _get_setup_updated_with_protocol_and_ppid(
                    SetupHolder.setup, is_exec=is_exec
                ),
    ...
    def new_fork():
        ...
        if not child_process:
            if is_new_python_process:
                ...
                _on_forked_process(
                    setup_tracing=apply_arg_patch and not is_subprocess_fork
                )
            else:
    ...
    if not IS_JYTHON:
        if not IS_WINDOWS:
            ...
            try:
                ...
                monkey_patch_module(
                    subprocess, "_fork_exec", create_subprocess_fork_exec
                )
    ...
    if not IS_JYTHON:
        if not IS_WINDOWS:
            ...
            try:
                ...
                monkey_patch_module(
                    _posixsubprocess, "fork_exec", create_warn_fork_exec
                )
            except ImportError:
                pass
            ...
            try:
                ...
                monkey_patch_module(
                    subprocess, "_fork_exec", create_subprocess_warn_fork_exec
                )
            except AttributeError:
                pass
            ...
            try:
                ...
                import _winapi as _subprocess
            monkey_patch_module(
                _subprocess, "CreateProcess", create_CreateProcessWarnMultiproc
            )
...
def patch_thread_module(thread_module):
    ...
    for attr in replace_attrs:
        if attr in start_joinable_attrs:
            if getattr(thread_module, "_original_start_joinable_thread", None) is None:
                _original_start_joinable_thread = (
                    thread_module._original_start_joinable_thread
                ) = getattr(thread_module, attr)
            else:
                _original_start_joinable_thread = (
                    thread_module._original_start_joinable_thread
                )
            ...
            if getattr(thread_module, "_original_start_new_thread", None) is None:
                _original_start_new_thread = (
                    thread_module._original_start_new_thread
                ) = getattr(thread_module, attr)
            else:
    ...
    class ClassWithPydevStartNewThread:
        def pydev_start_new_thread(self, function, args=(), kwargs={}):
            ...
            """
            return _original_start_new_thread(
                _UseNewThreadStartup(function, args, kwargs), ()
            )
    ...
    class ClassWithPydevStartJoinableThread:
        def pydev_start_joinable_thread(self, function, *args, **kwargs):
            ...
            return _original_start_joinable_thread(
                _UseNewThreadStartup(function, args, kwargs),
                handle=handle,
                daemon=daemon,
            )
