...
class ConcreteInstr(Instr):
    ...
    def __init__(
        self, name, arg=UNSET, *, lineno=None, extended_args=None, offset=None
    ):
...
class ConcreteBytecode(_bytecode._BaseBytecodeList):
    ...
    def from_code(code, *, extended_arg=False):
        line_starts = dict(
            entry for entry in dis.findlinestarts(code) if entry[1] is not None
        )
    ...
    def to_bytecode(self):
        ...
        for index, jump_target in jumps:
            ...
            instructions[index] = Instr(
                instr.name, label, lineno=instr.lineno, offset=instr.offset
            )
