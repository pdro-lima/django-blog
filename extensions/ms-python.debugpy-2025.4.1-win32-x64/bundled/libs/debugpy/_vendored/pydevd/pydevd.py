...
from _pydevd_bundle.pydevd_breakpoints import (
    ExceptionBreakpoint,
    get_exception_breakpoint,
)
...
from _pydevd_bundle.pydevd_defaults import (
    PydevdCustomization,
)  # Note: import alias used on pydev_monkey.
from _pydevd_bundle.pydevd_custom_frames import (
    CustomFramesContainer,
    custom_frames_container_init,
)
from _pydevd_bundle.pydevd_dont_trace_files import (
    DONT_TRACE,
    PYDEV_FILE,
    LIB_FILE,
    DONT_TRACE_DIRS,
...
from _pydevd_bundle.pydevd_frame_utils import (
    add_exception_to_frame,
    remove_exception_from_frame,
    short_stack,
)
...
from _pydevd_bundle.pydevd_concurrency_analyser.pydevd_thread_wrappers import (
    wrap_threads,
)
...
from _pydevd_bundle.pydevd_comm import (
    ...
    InternalThreadCommandForAnyThread,
...
from _pydevd_bundle.pydevd_daemon_thread import (
    PyDBDaemonThread,
    mark_as_pydevd_daemon_thread,
)
...
from _pydevd_bundle.pydevd_collect_bytecode_info import (
    collect_try_except_info,
    collect_return_info,
...
from _pydevd_bundle.pydevd_thread_lifecycle import (
    suspend_all_threads,
    mark_thread_suspended,
)
...
try:
    "dummy".encode(
        "utf-8"
    )  # Added because otherwise Jython 2.2.1 wasn't finding the encoding (if it wasn't loaded in the main thread).
except:
    pass
...
pydev_log.debug(
    "Using GEVENT_SHOW_PAUSED_GREENLETS: %s",
    pydevd_constants.GEVENT_SHOW_PAUSED_GREENLETS,
)
pydev_log.debug("pydevd __file__: %s", os.path.abspath(__file__))
pydev_log.debug(
    "Using PYDEVD_IPYTHON_COMPATIBLE_DEBUGGING: %s",
    pydevd_constants.PYDEVD_IPYTHON_COMPATIBLE_DEBUGGING,
)
if pydevd_constants.PYDEVD_IPYTHON_COMPATIBLE_DEBUGGING:
    pydev_log.debug(
        "PYDEVD_IPYTHON_CONTEXT: %s", pydevd_constants.PYDEVD_IPYTHON_CONTEXT
    )
            ...
            if self._suspended_thread_id_to_thread:
                if global_suspend_time > self._last_suspend_notification_time:
                    ...
                    thread_id, thread = next(
                        iter(self._suspended_thread_id_to_thread.items())
                    )
                    self.send_suspend_notification(
                        thread_id, thread, CMD_THREAD_SUSPEND
                    )
...
class ThreadsSuspendedSingleNotification(AbstractSingleNotificationBehavior):
    ...
    def send_resume_notification(self, thread_id):
        ...
        if py_db is not None:
            py_db.writer.add_command(
                py_db.cmd_factory.make_thread_resume_single_notification(thread_id)
            )
    ...
    def notify_thread_suspended(self, thread_id, thread, stop_reason):
        if self.multi_threads_single_notification:
            ...
            with AbstractSingleNotificationBehavior.notify_thread_suspended(
...
class _Authentication(object):
    ...
    def login(self, access_token):
        if (
            self._wrong_attempts >= 10
        ):  # A user can fail to authenticate at most 10 times.
            return
...
class PyDB(object):
    ...
    def __init__(self, set_as_global=True):
        ...
        self._cmd_queue = defaultdict(
            _queue.Queue
        )  # Key is thread id or '*', value is Queue
        self._thread_events = defaultdict(
            ThreadingEvent
        )  # Key is thread id or '*', value is Event
        ...
        self.source_mapping = SourceMapping(
            on_source_mapping_changed=self._clear_caches
        )
        ...
        if set_as_global:
            CustomFramesContainer._py_db_command_thread_event = (
                self._py_db_command_thread_event
            )
        ...
        self.is_files_filter_enabled = (
            self._exclude_filters_enabled or self._is_libraries_filter_enabled
        )
        ...
        # If True, pydevd will send a single notification when all threads are suspended/resumed.
        self._threads_suspended_single_notification = (
            ThreadsSuspendedSingleNotification(self)
        )
        ...
        self.fix_top_level_trace_and_get_trace_func = (
            fix_top_level_trace_and_get_trace_func
        )
    ...
    def collect_try_except_info(self, code_obj):
        ...
        try:
            if os.path.exists(filename):
                pydev_log.debug(
                    "Collecting try..except info from source for %s", filename
                )
                ...
                if try_except_infos:
                    ...
                    try_except_infos = [
                        x
                        for x in try_except_infos
                        if min_line <= x.try_line <= max_line
                    ]
                return try_except_infos

        except:
            pydev_log.exception(
                "Error collecting try..except info from source (%s)", filename
            )

        pydev_log.debug("Collecting try..except info from bytecode for %s", filename)
        return collect_try_except_info(code_obj)

    def setup_auto_reload_watcher(
        self,
        enable_auto_reload,
        watch_dirs,
        poll_target_time,
    ...
    def handle_breakpoint_condition(self, info, pybreakpoint, new_frame):
        ...
        try:
            ...
            if not isinstance(e, self.skip_print_breakpoint_exception):
                ...
                msg = (
                    "Error while evaluating expression in conditional breakpoint: %s\n%s"
    ...
    def handle_breakpoint_expression(self, pybreakpoint, info, new_frame):
        try:
            try:
                val = eval(
                    pybreakpoint.expression, new_frame.f_globals, new_frame.f_locals
                )
            except:
                val = sys.exc_info()[1]
        finally:
    ...
    def _internal_get_file_type(self, abs_real_path_and_basename):
        ...
        if basename.startswith(
            IGNORE_BASENAMES_STARTING_WITH
        ) or abs_real_path_and_basename[0].startswith(IGNORE_BASENAMES_STARTING_WITH):
        ...
        if (
            basename.startswith("__init__.py")
            or basename in LIB_FILES_IN_DONT_TRACE_DIRS
        ):
    ...
    def get_file_type(
        self, frame, abs_real_path_and_basename=None, _cache_file_type=_CACHE_FILE_TYPE
    ):
        """
        ...
        if abs_real_path_and_basename is None:
            try:
                ...
                abs_real_path_and_basename = NORM_PATHS_AND_BASE_CONTAINER[
                    frame.f_code.co_filename
                ]
            except:
                abs_real_path_and_basename = get_abs_path_real_path_and_base_from_frame(
                    frame
                )
        ...
        try:
            ...
            if abs_real_path_and_basename[0] == "<string>":
                f = frame.f_back
                while f is not None:
                    if self.get_file_type(
                        f
                    ) != self.PYDEV_FILE and pydevd_file_utils.basename(
    ...
    def set_tracing_for_untraced_contexts(self, breakpoints_changed=False):
        ...
        if IS_CPYTHON:
            ...
            try:
                ...
                for t in threads:
                    if getattr(t, "is_pydev_daemon_thread", False) or getattr(
                        t, "pydev_do_not_trace", False
                    ):
                        continue
    ...
    def in_project_scope(self, frame, absolute_filename=None):
        ...
        try:
            ...
            if file_type == self.PYDEV_FILE:
                ...
                cache[cache_key] = self._files_filtering.in_project_roots(
                    absolute_filename
                )
    ...
    def _exclude_by_filter(self, frame, absolute_filename):
        ...
        try:
            ...
            if self.get_file_type(frame) == self.PYDEV_FILE:
                ...
                cache[cache_key] = self._files_filtering.exclude_by_filter(
                    absolute_filename, module_name
                )
        ...
        try:
            ...
            if self.plugin is not None and (
                ...
                if not self.plugin.can_skip(self, frame):
                    if DEBUG:
                        pydev_log.debug_once(
                            "File traced (included by plugins): %s", original_filename
                        )
            ...
            if self._exclude_filters_enabled:
                ...
                if exclude_by_filter is not None:
                    if exclude_by_filter:
                        ...
                        if DEBUG:
                            pydev_log.debug_once(
                                "File not traced (excluded by filters): %s",
                                original_filename,
                            )
                        ...
                        if DEBUG:
                            pydev_log.debug_once(
                                "File traced (explicitly included by filters): %s",
                                original_filename,
                            )
            ...
            if (
                self._is_libraries_filter_enabled or force_check_project_scope
            ) and not self.in_project_scope(frame):
                # ignore library files while stepping
                self._apply_filter_cache[cache_key] = True
                if force_check_project_scope:
                    if DEBUG:
                        pydev_log.debug_once(
                            "File not traced (not in project): %s", original_filename
                        )
                else:
                    if DEBUG:
                        pydev_log.debug_once(
                            "File not traced (not in project - force_check_project_scope): %s",
                            original_filename,
                        )
            ...
            if force_check_project_scope:
                if DEBUG:
                    pydev_log.debug_once(
                        "File traced: %s (force_check_project_scope)", original_filename
                    )
            else:
                if DEBUG:
    ...
    def exclude_exception_by_filter(self, exception_breakpoint, trace):
        if (
            not exception_breakpoint.ignore_libraries
            and not self._exclude_filters_enabled
        ):
            return False
    ...
    def initialize_network(self, sock, terminate_on_socket_close=True):
        ...
        try:
            sock.settimeout(
                None
            )  # infinite, no timeouts from now on - jython does not have it
        except:
            pass
        ...
        self.writer = WriterThread(
            sock, self, terminate_on_socket_close=terminate_on_socket_close
        )
    ...
    def create_wait_for_connection_thread(self):
        if self._waiting_for_connection_thread is not None:
            raise AssertionError(
                "There is already another thread waiting for a connection."
            )
        ...
        def run(self):
            ...
            while not self._kill_received:
                try:
                    ...
                    if self._kill_received:
                        pydev_log.info(
                            "Connection (from wait_for_attach) accepted but ignored as kill was already received."
                        )
                        return
                    ...
                    self.py_db.initialize_network(
                        new_socket, terminate_on_socket_close=False
                    )
    ...
    def get_internal_queue_and_event(
        self, thread_id
    ) -> Tuple[_queue.Queue, ThreadingEvent]:
    ...
    def post_method_as_internal_command(self, thread_id, method, *args, **kwargs):
        if thread_id == "*":
            internal_cmd = InternalThreadCommandForAnyThread(
                thread_id, method, *args, **kwargs
            )
        else:
    ...
    def init_gui_support(self):
        ...
        def return_control():
            ...
            _ReturnGuiLoopControlHelper._return_control_osc = (
                not _ReturnGuiLoopControlHelper._return_control_osc
            )
    ...
    def _activate_gui_if_needed(self):
        ...
        if len(self.mpl_modules_for_patching) > 0:
            if (
                is_current_thread_main_thread()
            ):  # Note that we call only in the main thread.
                for module in list(self.mpl_modules_for_patching):
                    if module in sys.modules:
                        activate_function = self.mpl_modules_for_patching.pop(
                            module, None
                        )
        ...
        if self.activate_gui_function:
            if (
                is_current_thread_main_thread()
            ):  # Only call enable_gui in the main thread.
                try:
                    ...
                    try:
                        ...
                        pydev_log.debug(
                            "Cannot activate custom GUI event loop {}: {}".format(
                                self._gui_event_loop, e
                            )
                        )
                    finally:
                        self.activate_gui_function = None
    ...
    def notify_skipped_step_in_because_of_filters(self, frame):
        self.writer.add_command(
            self.cmd_factory.make_skipped_step_in_because_of_filters(self, frame)
        )
                        ...
                        while True:
                            ...
                            try:
                                if internal_cmd.can_be_executed_by(curr_thread_id):
                                    ...
                                    pydev_log.verbose(
                                        "NOT processing internal command: %s ",
                                        internal_cmd,
                                    )
    ...
    def add_break_on_exception(
        ...
        try:
            ...
            pydev_log.critical(
                "Error unable to add break on exception for: %s (exception could not be imported).",
                exception,
            )
            return None
    ...
    def set_suspend(
        ...
        info = mark_thread_suspended(
            thread, stop_reason, original_step_cmd=original_step_cmd
        )
        ...
        if (
            ...
            and info.conditional_breakpoint_exception is not None
        ):
            conditional_breakpoint_exception_tuple = (
                info.conditional_breakpoint_exception
            )
            info.conditional_breakpoint_exception = None
            self._send_breakpoint_condition_exception(
                thread, conditional_breakpoint_exception_tuple
            )
    ...
    def _send_breakpoint_condition_exception(
        ...
        if (
            conditional_breakpoint_exception_tuple
            and len(conditional_breakpoint_exception_tuple) == 2
        ):
    ...
    def send_process_about_to_be_replaced(self):
        ...
        while not sent[0]:
            ...
            if (time.time() - initial_time) > timeout:
                pydev_log.critical(
                    "pydevd: Sending message related to process being replaced timed-out after %s seconds",
                    timeout,
                )
                break
    ...
    def do_wait_suspend(
        ...
        thread.additional_info.trace_suspend_type = (
            "trace"  # Reset to trace mode for next call.
        )
        ...
        with self.suspended_frames_manager.track_frames(self) as frames_tracker:
            ...
            with CustomFramesContainer.custom_frames_lock:  # @UndefinedVariable
                ...
                for (
                    ...
                    custom_frame,
                ) in CustomFramesContainer.custom_frames.items():
                    if custom_frame.thread_id == thread.ident:
                        frames_tracker.track(
                            thread_id,
                            pydevd_frame_utils.create_frames_list_from_frame(
                                custom_frame.frame
                            ),
                        ...
                        self.writer.add_command(
                            self.cmd_factory.make_custom_frame_created_message(
            ...
            with self._threads_suspended_single_notification.notify_thread_suspended(
        ...
        if DebugInfoHolder.DEBUG_TRACE_LEVEL > 2:
            pydev_log.debug(
                "Leaving PyDB.do_wait_suspend: %s (%s) %s",
                thread,
                thread_id,
                id(thread),
            )

    def _do_wait_suspend(
        self,
        thread,
        frame,
        event,
        ...
        try:
            ...
            with self._main_lock:  # Use lock to check if suspended state changed
                activate_gui = (
                    info.pydev_state == STATE_SUSPEND and not self.pydb_disposed
                )
            ...
            while True:
                with self._main_lock:  # Use lock to check if suspended state changed
                    if info.pydev_state != STATE_SUSPEND or (
                        self.pydb_disposed and not self.terminate_requested
                    ):
                ...
                try:
                    ...
                    if internal_cmd.can_be_executed_by(curr_thread_id):
                        pydev_log.verbose(
                            "processing internal command: %s", internal_cmd
                        )
                        try:
                            internal_cmd.do_it(self)
                        except:
        ...
        if info.pydev_step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE):
            ...
            info.pydev_step_cmd == CMD_RUN_TO_LINE
            or info.pydev_step_cmd == CMD_SET_NEXT_STATEMENT
        ):
            info.pydev_step_stop = None
            self.set_trace_for_frame_and_parents(thread.ident, frame)
            stop = False
            response_msg = ""
            try:
                stop, _old_line, response_msg = self.set_next_statement(
                    frame, event, info.pydev_func_name, info.pydev_next_line
                )
            except ValueError as e:
                response_msg = "%s" % e
            finally:
                seq = info.pydev_message
                cmd = self.cmd_factory.make_set_next_stmnt_status_message(
                    seq, stop, response_msg
                )
                self.writer.add_command(cmd)
                info.pydev_message = ""
            ...
            if stop:
                ...
                cmd = self.cmd_factory.make_thread_run_message(
                    self, get_current_thread_id(thread), info.pydev_step_cmd
                )
                self.writer.add_command(cmd)
        ...
        with CustomFramesContainer.custom_frames_lock:
            ...
            for frame_id in from_this_thread:
                ...
                self.writer.add_command(
                    self.cmd_factory.make_thread_killed_message(frame_id)
                )
    ...
    def do_stop_on_unhandled_exception(self, thread, frame, frames_byid, arg):
        ...
        try:
            ...
            try:
                ...
                self.do_wait_suspend(
                    thread, frame, "exception", arg, EXCEPTION_TYPE_UNHANDLED
                )
            except:
                self.send_caught_exception_stack_proceeded(thread)
        except:
            pydev_log.exception(
                "We've got an error while stopping in unhandled exception: %s.", arg[0]
            )
        finally:
            remove_exception_from_frame(frame)
            frame = None

    def set_trace_for_frame_and_parents(
            ...
            if PYDEVD_USE_SYS_MONITORING:
                if file_type is None:
                    if disable:
                        ...
                        if DEBUG:
                            pydev_log.debug(
                                "Set tracing of frame: %s - %s",
                                frame.f_code.co_filename,
                                frame.f_code.co_name,
                            )
                        pydevd_sys_monitoring.enable_code_tracing(
                            thread_ident, frame.f_code, frame
                        )
                else:
                    if DEBUG:
                        pydev_log.debug(
                            "SKIP set tracing of frame: %s - %s",
                            frame.f_code.co_filename,
                            frame.f_code.co_name,
                        )
            else:
    ...
    def __wait_for_threads_to_finish(self, timeout):
        try:
            with self._wait_for_threads_to_finish_called_lock:
                wait_for_threads_to_finish_called = (
                    self._wait_for_threads_to_finish_called
                )
            ...
            if wait_for_threads_to_finish_called:
                ...
                try:
                    ...
                    while time.time() < started_at + timeout:
                        ...
                        thread_names = [
                            t.name for t in get_pydb_daemon_threads_to_wait()
                        ]
                        if thread_names:
                            pydev_log.debug(
                                "The following pydb threads may not have finished correctly: %s",
                                ", ".join(thread_names),
                            )
                finally:
        ...
        try:
            ...
            if disposed:
                if wait:
                    ...
                    pydev_log.debug(
                        "PyDB.dispose_and_kill_all_pydevd_threads (already disposed - no wait)"
                    )
                return
            ...
            for t in pydb_daemon_threads:
                if hasattr(t, "do_kill_pydev_thread"):
                    pydev_log.debug(
                        "PyDB.dispose_and_kill_all_pydevd_threads killing thread: %s", t
                    )
    ...
    def _exec(self, is_module, entry_point_fn, module_name, file, globals, locals):
        ...
        )  # Keep in 't' local variable to be accessed afterwards from frame.f_locals.
        if not is_module:
            ...
            if entry_point_fn:
                ...
                globals = pydevd_runpy._run_module_as_main(
                    module_name, alter_argv=False
                )
        return globals
    ...
    def wait_for_commands(self, globals):
        ...
        frame = pydevd_frame_utils.Frame(
            None,
            -1,
            pydevd_frame_utils.FCode(
                "Console", os.path.abspath(os.path.dirname(__file__))
            ),
            globals,
            globals,
        )
...
def start_dump_threads_thread(filename_template, timeout, recurrent):
    ...
    """
    assert filename_template.count("%s") == 1, "Expected one %%s to appear in: %s" % (
        filename_template,
    )
...
def _enable_attach(
    ...
    if SetupHolder.setup is not None:
        if port != SetupHolder.setup["port"]:
            raise AssertionError(
                "Unable to listen in port: %s (already listening in port: %s)"
                % (port, SetupHolder.setup["port"])
            )
    settrace(
        host=host,
...
def _wait_for_attach(cancel=None):
    ...
    if py_db is None:
        raise AssertionError(
            "Debugger still not created. Please use _enable_attach() before using _wait_for_attach()."
        )
...
def settrace(
    ...
    **kwargs,
    ...
    stdout_to_server = stdout_to_server or kwargs.get(
        "stdoutToServer", False
    )  # Backward compatibility
    stderr_to_server = stderr_to_server or kwargs.get(
...
def _locked_settrace(
    ...
    if py_db is None:
        ...
        if dont_trace_start_patterns or dont_trace_end_patterns:
            PyDevdAPI().set_dont_trace_start_end_patterns(
                py_db, dont_trace_start_patterns, dont_trace_end_patterns
            )
...
class Dispatcher(object):
    ...
    def connect(self, host, port):
        ...
        self.reader.pydev_do_not_trace = (
            False  # we run reader in the same thread so we don't want to loose tracing
        )
        self.reader.run()
...
def settrace_forked(setup_tracing=True):
    ...
    if py_db is not None:
        py_db.created_pydb_daemon_threads = (
            {}
        )  # Just making sure we won't touch those (paused) threads.
        py_db = None
...
def apply_debugger_options(setup_options):
    ...
    if setup_options["save-signatures"]:
        if pydevd_vm_type.get_vm_type() == pydevd_vm_type.PydevdVmType.JYTHON:
            sys.stderr.write(
                "Collecting run-time type information is not supported for Jython\n"
            )
        else:
...
def _log_initial_info():
    ...
    pydev_log.debug(
        "Using gevent mode: %s / imported gevent module support: %s",
        SUPPORT_GEVENT,
        bool(pydevd_gevent_integration),
    )


def config(protocol="", debug_mode="", preimport=""):
    pydev_log.debug(
        "Config: protocol: %s, debug_mode: %s, preimport: %s",
        protocol,
        debug_mode,
        preimport,
    )
...
def main():
        ...
        if setup["multiprocess"]:  # PyDev
            ...
            try:
                ...
                if dispatcher.port is not None:
                    ...
                    pydev_log.info(
                        "pydev debugger: process %d is connecting\n" % os.getpid()
                    )
                    ...
                    pydev_log.critical(
                        "pydev debugger: couldn't get port for new debug process."
                    )
            finally:
                dispatcher.close()
        else:
    ...
    if fix_app_engine_debug:
        ...
        app_engine_startup_file = os.path.join(
            curr_dir, "pydev_app_engine_debug_startup.py"
        )
