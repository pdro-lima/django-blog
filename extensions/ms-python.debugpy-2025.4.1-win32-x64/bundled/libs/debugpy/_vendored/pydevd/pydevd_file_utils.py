        ...
        if resolved_joined is None:
            ...
            for filename in dir_contents:
                ...
                raise FileNotFoundError(
                    "Unable to find: %s in %s. Dir Contents: %s"
...
DEBUG_CLIENT_SERVER_TRANSLATION = os.environ.get(
    "DEBUG_PYDEVD_PATHS_TRANSLATION", "False"
).lower() in ("1", "true")
...
def set_ide_os(os):
    ...
    if (
        os == "WIN"
    ):  # Apparently PyCharm uses 'WIN' (https://github.com/fabioz/PyDev.Debugger/issues/116)
        os = "WINDOWS"
    ...
    try:
        ...
        if filename.__class__ != str:
            raise AssertionError(
                "Paths passed to _abs_and_canonical_path must be str. Found: %s (%s)"
        ...
        if (
            os_path_abspath is None
            or os_path_isabs is None
            or os_path_real_path is None
        ...
        normalize = False
        abs_path = _apply_func_and_normalize_case(
            filename, os_path_abspath, isabs, normalize
        )

        normalize = True
        real_path = _apply_func_and_normalize_case(
            filename, os_path_real_path, isabs, normalize
        )
...
def _apply_func_and_normalize_case(
...
try:
    ...
    if code.co_filename.startswith("<frozen"):
        ...
        report(
            "Debugger warning: The os.path.realpath.__code__.co_filename (%s)",
            code.co_filename,
        )
        ...
        report(
            "Debugger warning: It seems the debugger cannot find os.path.realpath.__code__.co_filename (%s).",
            code.co_filename,
        )
...
def _original_file_to_client(filename, cache={}):
    try:
        ...
        translated = _path_to_expected_str(
            get_path_with_real_case(absolute_path(filename))
        )
...
def create_source_reference_for_linecache(server_filename):
    ...
    pydev_log.debug(
        "Created linecache id source reference: %s for server filename: %s",
        source_reference,
        server_filename,
    )
...
def create_source_reference_for_frame_id(frame_id, original_filename):
    source_reference = _next_source_reference()
    pydev_log.debug(
        "Created frame id source reference: %s for frame id: %s (%s)",
        source_reference,
        frame_id,
        original_filename,
    )
...
def setup_client_server_paths(paths):
    ...
    for i, (path0, path1) in enumerate(paths):
        ...
        if not force_only_slash:
            ...
            paths_from_eclipse_to_python.append(
                (_normcase_from_client(path0), normcase(path1))
            )
        ...
        paths_from_eclipse_to_python_with_end_sep.append(
            (_normcase_from_client(path0), normcase(path1))
        )
    ...
    if DEBUG_CLIENT_SERVER_TRANSLATION:
        pydev_log.critical(
            ...
            ", ".join(
                ['"%s=>%s"' % (x[0], x[1]) for x in paths_from_eclipse_to_python]
            ),
        )
    ...
    paths_from_eclipse_to_python = (
        paths_from_eclipse_to_python_with_end_sep + paths_from_eclipse_to_python
    )
        ...
        try:
            ...
            for eclipse_prefix, server_prefix in paths_from_eclipse_to_python:
                if translated_normalized.startswith(eclipse_prefix):
                    ...
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        pydev_log.critical(
                            "pydev debugger: replacing to server: %s", filename
                        )
                ...
                if not os_path_exists(translated):
                    if not translated.startswith("<"):
                        ...
                        error_once(
                            ...
                            filename,
                            ", ".join(
                                ['"%s"' % (x[0],) for x in paths_from_eclipse_to_python]
                            ),
                        )
                else:
    ...
    def _map_file_to_client(filename, cache=norm_filename_to_client_container):
        ...
        try:
            ...
            for i, (eclipse_prefix, python_prefix) in enumerate(
                paths_from_eclipse_to_python
            ):
                if translated_normalized.startswith(python_prefix):
                    ...
                    translated = (
                        eclipse_prefix + translated_proper_case[len(python_prefix) :]
                    )
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        pydev_log.critical(
                            "pydev debugger: sent to client: %s - matched prefix: %s",
                            translated,
                            python_prefix,
                        )
...
def get_abs_path_real_path_and_base_from_frame(
