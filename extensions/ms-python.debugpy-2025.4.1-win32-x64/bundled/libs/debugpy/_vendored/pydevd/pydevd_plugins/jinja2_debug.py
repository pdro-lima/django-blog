...
class Jinja2LineBreakpoint(LineBreakpointWithLazyValidation):
    ...
    def __str__(self):
        return "Jinja2LineBreakpoint: %s-%d" % (
...
def after_breakpoints_consolidated(
    ...
    jinja2_breakpoints_for_file = file_to_line_to_breakpoints.get(
        canonical_normalized_filename
    )
...
def _is_jinja2_render_call(frame):
    try:
        ...
        if (
            "__jinja_template__" in frame.f_globals
            and name in ("root", "loop", "macro")
...
def _find_render_function_frame(frame):
    ...
    try:
        while not (
            ...
            and frame.f_locals["self"].__class__.__name__ == "Template"
            and frame.f_code.co_name == "render"
        ):
            frame = frame.f_back
            if frame is None:
                return old_frame
...
def can_skip(pydb, frame):
    if pydb.jinja2_breakpoints and _is_jinja2_render_call(frame):
        ...
        if filename is not None:
            ...
            jinja2_breakpoints_for_file = pydb.jinja2_breakpoints.get(
                canonical_normalized_filename
            )
...
def cmd_step_into(pydb, frame, event, info, thread, stop_info, stop):
    ...
    if _is_jinja2_suspended(thread):
        plugin_stop = stop_info["jinja2_stop"] = event in (
            "call",
            "line",
        ) and _is_jinja2_render_call(frame)
        stop = False
...
def cmd_step_over(pydb, frame, event, info, thread, stop_info, stop):
    ...
    if _is_jinja2_suspended(thread):
        ...
        if info.pydev_call_inside_jinja2 is None:
            ...
            if event == "line":
                if (
                    _is_jinja2_render_call(frame)
                    and info.pydev_call_inside_jinja2 is frame
                ):
                    plugin_stop = stop_info["jinja2_stop"] = True
            if event == "return":
                if (
                    frame is info.pydev_call_inside_jinja2
                    and "event" not in frame.f_back.f_locals
                ):
...
def get_breakpoint(py_db, frame, event, info):
    ...
    if (
        ...
        if original_filename is not None:
            ...
            jinja2_breakpoints_for_file = py_db.jinja2_breakpoints.get(
                canonical_normalized_filename
            )

            if jinja2_breakpoints_for_file:
                jinja2_validation_info = py_db.jinja2_validation_info
                jinja2_validation_info.verify_breakpoints(
                    py_db,
                    canonical_normalized_filename,
                ...
                if template_lineno is not None:
                    ...
                    if jinja2_breakpoint is not None:
                        new_frame = Jinja2TemplateFrame(
                            frame, original_filename, template_lineno
                        )
