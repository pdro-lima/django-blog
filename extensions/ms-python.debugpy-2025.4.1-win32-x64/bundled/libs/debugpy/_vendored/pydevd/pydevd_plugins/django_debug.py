...
class DjangoLineBreakpoint(LineBreakpointWithLazyValidation):
    ...
    def __str__(self):
        return "DjangoLineBreakpoint: %s-%d" % (
...
def add_line_breakpoint(
    ...
    if type == "django-line":
        ...
        if IS_DJANGO19_OR_HIGHER:
            ...
            django_line_breakpoint.on_changed_breakpoint_state = (
...
def after_breakpoints_consolidated(
    py_db,
    canonical_normalized_filename,
    id_to_pybreakpoint,
    ...
    if IS_DJANGO19_OR_HIGHER:
        django_breakpoints_for_file = file_to_line_to_breakpoints.get(
            canonical_normalized_filename
        )
                ...
                if hasattr(context, "_has_included_template"):
                    ...
                    while back is not None and frame.f_code.co_name in (
                        ...
                        if "self" in locals:
                            self = locals["self"]
                            if (
                                self.__class__.__name__ == "Template"
                                and hasattr(self, "origin")
                                and hasattr(self.origin, "name")
                            ):
...
def cmd_step_into(py_db, frame, event, info, thread, stop_info, stop):
    ...
    if _is_django_suspended(thread):
        plugin_stop = stop_info["django_stop"] = (
            event == "call" and _is_django_render_call(frame)
        )
        stop = (
            stop
            and _is_django_resolve_call(frame.f_back)
            and not _is_django_context_get_call(frame)
        )
        if stop:
...
def cmd_step_over(py_db, frame, event, info, thread, stop_info, stop):
    ...
    if _is_django_suspended(thread):
        ...
        if (
            event == "return"
            and info.pydev_django_resolve_frame
            and _is_django_resolve_call(frame.f_back)
        ):
...
def get_breakpoint(py_db, frame, event, info):
    ...
    if (
        ...
        if django_breakpoints_for_file:
            ...
            pydev_log.debug(
                "Breakpoints for that file: %s", django_breakpoints_for_file
            )
...
def exception_break(py_db, frame, thread, arg, is_unwind):
    ...
    if py_db.django_exception_break and exception is not None:
        if (
            exception.__name__
            in ["VariableDoesNotExist", "TemplateDoesNotExist", "TemplateSyntaxError"]
            and not is_unwind
            ...
            if exception.__name__ == "TemplateSyntaxError":
                ...
                if original_filename is not None and lineno is not None:
                    ...
                    suspend_frame = suspend_django(
                        py_db, thread, syntax_error_frame, CMD_ADD_EXCEPTION_BREAK
                    )
                ...
                if _is_django_variable_does_not_exist_exception_break_context(frame):
                    if not getattr(
                        exception, "silent_variable_failure", False
                    ) and not _is_ignoring_failures(frame):
                        ...
                        if render_frame:
                            ...
                            if suspend_frame:
                                add_exception_to_frame(
                                    suspend_frame, (exception, value, trace)
                                )
                                thread.additional_info.pydev_message = (
