...
def parse_cmdline(argv=None):
        ...
        if opt in ("-v", "--verbosity"):
            ...
            if split_jobs not in ("module", "tests"):
                raise AssertionError(
                    'Expected split to be either "module" or "tests". Was :%s'
                    % (split_jobs,)
                )
    ...
    if tests:
        if verbosity > 4:
            sys.stdout.write(
                "--tests provided. Ignoring --exclude_files, --exclude_tests and --include_files\n"
            )
...
class PydevTestRunner(object):
                    ...
                    for root, dirs, files in os.walk(base_dir):
                        ...
                        for d in dirs:
                            for init in [
                                ...
                                if os.path.exists(
                                    os.path.join(root, d, init).replace("\\", "/")
                                ):
                                    break
                            else:
                                exclude[d] = 1
                ...
                if not add:
                    ...
                    for pat in self.configuration.exclude_files:
                        if fnmatch.fnmatchcase(basename, pat):
                            if self.verbosity > 3:
                                sys.stdout.write(
                                    "Skipped file: %s (matched exclude_files pattern: %s)\n"
                                    % (f, pat)
                                )
        ...
        * Returns the "final" module, so importing "coilib40.subject.visu"
        ...
        try:
            ...
            if print_exception:
                ...
                buf_err = pydevd_io.start_redirect(
                    keep_original_redirection=True, std="stderr"
                )
                buf_out = pydevd_io.start_redirect(
        ...
        for pyfile, imp in imports:
            ...
            if not choices:
                ...
                for i, import_str in enumerate(choices):
                    ...
                    mod = self.__get_module_from_str(
                        import_str, print_exception, pyfile
                    )
                    if mod is not None:
        ...
        def __call__(self, testCaseClass):
            ...
            if className in self.accepted_classes:
                for attrname in dir(testCaseClass):
                    ...
                    if attrname.startswith("test") and hasattr(
                        getattr(testCaseClass, attrname), "__call__"
                    ):
    ...
    def find_tests_from_modules(self, file_and_modules_and_module_name):
        ...
        if self.files_to_tests:
            for pyfile, m, module_name in file_and_modules_and_module_name:
                ...
                loader.getTestCaseNames = self.GetTestCaseNames(
                    accepted_classes, accepted_methods
                )
    ...
    def filter_tests(self, test_objs, internal_call=False):
        ...
        if not internal_call:
            if (
                not self.configuration.include_tests
                and not self.tests
                and not self.configuration.exclude_tests
            ):
        ...
        for test_obj in test_objs:
            if isinstance(test_obj, unittest.TestSuite):
                ...
                if test_obj._tests:
                    ...
                    if (
                        test_obj._tests
                    ):  # Only add the suite if we still have tests there.
                ...
                if self.configuration.exclude_tests:
                    for pat in self.configuration.exclude_tests:
                        if fnmatch.fnmatchcase(testMethodName, pat):
                            if self.verbosity > 3:
                                sys.stdout.write(
                                    "Skipped test: %s (matched exclude_tests pattern: %s)\n"
                                    % (testMethodName, pat)
                                )
    ...
    def __match_tests(self, tests, test_case, test_method_name):
        ...
        for t in tests:
            ...
            if len(class_and_method) == 1:
                ...
                if (
                    class_and_method[0] == test_case.__class__.__name__
                    and class_and_method[1] == test_method_name
                ):
                    return 1
        ...
        def run_tests():
            ...
            if self.jobs > 1:
                ...
                executed_in_parallel = (
                    pydev_runfiles_parallel.execute_tests_in_parallel(
                        all_tests,
                        self.jobs,
                        self.split_jobs,
                        self.verbosity,
                        coverage_files,
                        self.configuration.coverage_include,
                    )
                )
            ...
            if not executed_in_parallel:
                ...
                runner = pydev_runfiles_unittest.PydevTextTestRunner(
                    stream=sys.stdout, descriptions=1, verbosity=self.verbosity
                )
                sys.stdout.write("\n")
