    ...
    if py_test_accept_filter is None:
        ...
        if py_test_accept_filter:
            ...
            for filename, value in py_test_accept_filter.items():
                new_dct[canonical_normalized_path(str(Path(filename).resolve()))] = (
                    value
                )
...
def start_redirect():
    ...
    State.buf_err = pydevd_io.start_redirect(
        keep_original_redirection=True, std="stderr"
    )
    State.buf_out = pydevd_io.start_redirect(
...
def get_curr_output():
    ...
    return buf_out.getvalue() if buf_out is not None else "", (
        buf_err.getvalue() if buf_err is not None else ""
    )
    ...
    for item in items:
        ...
        for test in accept_tests:
            ...
            if (
                ...
                and test == name_without_parametrize
            ):
            ...
            if class_name is not None:
                ...
                if (
                    name_without_parametrize is not None
                    and test == class_name + "." + name_without_parametrize
                ):
                    new_items.append(item)
                    break
...
def pytest_collectreport(report):
    ...
    if error_contents:
        report_test(
            "fail", "<collect errors>", "<collect errors>", "", error_contents, 0.0
        )
...
def pytest_runtest_logreport(report):
    ...
    # come from here...
    captured_output, error_contents = getattr(
        report, "pydev_captured_output", ""
    ...
    report_test(
        status, filename, test, captured_output, error_contents, report_duration
    )
...
if not hasattr(pytest, "hookimpl"):
    raise AssertionError(
        "Please upgrade pytest (the current version of pytest: %s is unsupported)"
        % (pytest.__version__,)
    )
