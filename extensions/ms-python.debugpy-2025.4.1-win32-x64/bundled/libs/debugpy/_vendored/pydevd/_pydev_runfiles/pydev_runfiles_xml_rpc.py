...
class ServerFacade(object):
    ...
    def notifyTestsCollected(self, *args):
        self.notifications_queue.put_nowait(
            ParallelNotification("notifyTestsCollected", args)
        )

    def notifyConnected(self, *args):
        self.notifications_queue.put_nowait(
            ParallelNotification("notifyConnected", args)
        )

    def notifyTestRunFinished(self, *args):
        self.notifications_queue.put_nowait(
            ParallelNotification("notifyTestRunFinished", args)
        )

    def notifyStartTest(self, *args):
        self.notifications_queue.put_nowait(
            ParallelNotification("notifyStartTest", args)
        )
...
class ServerComm(threading.Thread):
    def __init__(self, notifications_queue, port, daemon=False):
        ...
        self.server = xmlrpclib.Server(
            "http://%s:%s" % (pydev_localhost.get_localhost(), port), encoding=encoding
        )
    ...
    def run(self):
        while True:
            ...
            try:
                while True:
                    command = self.notifications_queue.get(
                        block=False
                    )  # No block to create a batch.
                    if isinstance(command, KillServer):
                        kill_found = True
                    else:
                        assert isinstance(command, ParallelNotification)
    ...
    if isinstance(obj, str):  # Unicode in py3
        ...
        try:
            return xmlrpclib.Binary(
                obj.decode(sys.stdin.encoding, "replace").encode(
...
def notifyTest(cond, captured_output, error_contents, file, test, time):
    ...
    try:
        ...
        _ServerHolder.SERVER.notifyTest(
            cond, captured_output, error_contents, file, test, time
        )
    except:
        traceback.print_exc()
